...
-- 3. 确保租赁管理员可以查看自己的profile
-- 这个策略应该已经存在，但我们重新创建以确保
...
-- 4. 确保租赁管理员可以更新自己的profile
DROP POLICY IF EXISTS "租赁管理员可以更新自己的profile" ON profiles;
CREATE POLICY "租赁管理员可以更新自己的profile" ON profiles
  FOR UPDATE TO authenticated
  USING (
    auth.uid() = id AND is_lease_admin()
  )
  WITH CHECK (
    auth.uid() = id AND is_lease_admin()
  );
