/*
# 权限系统重构 - 步骤4：应用新的 RLS 策略（users 表）- 修正版

## 说明
为 users 表应用基于策略模板的动态 RLS 策略，替换原有的硬编码策略

## 修正
使用正确的表名：warehouse_assignments（而不是 driver_warehouses）
...
-- 删除可能存在的新策略（如果之前创建过）
DROP POLICY IF EXISTS "new_admins_view_all_users" ON users;
DROP POLICY IF EXISTS "new_managers_view_their_drivers" ON users;
DROP POLICY IF EXISTS "new_drivers_view_self" ON users;
DROP POLICY IF EXISTS "new_admins_insert_users" ON users;
DROP POLICY IF EXISTS "new_admins_update_all_users" ON users;
DROP POLICY IF EXISTS "new_drivers_update_self" ON users;
DROP POLICY IF EXISTS "new_admins_delete_users" ON users;
