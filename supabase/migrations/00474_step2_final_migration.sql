...
- 对于可选的外键（如 sender_id），如果不存在则设为 NULL
*/
...
SELECT 
  ...
  CASE 
    WHEN EXISTS (SELECT 1 FROM users WHERE id = l.reviewed_by) THEN l.reviewed_by
    ELSE NULL
  END as approver_id,
...
-- 13. 迁移通知数据（sender_id 不存在则设为 NULL）
...
SELECT 
  ...
  CASE 
    WHEN EXISTS (SELECT 1 FROM users WHERE id = n.sender_id) THEN n.sender_id
    ELSE NULL
  END as sender_id,
  n.recipient_id,
