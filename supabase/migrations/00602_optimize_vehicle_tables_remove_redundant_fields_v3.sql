...
BEGIN
  ...
  SELECT COUNT(*) INTO d_col_count 
  FROM information_schema.columns 
  WHERE table_schema = 'public' AND table_name = 'vehicle_documents';
  
  SELECT COUNT(*) INTO v_rec_count FROM vehicles;
  SELECT COUNT(*) INTO d_rec_count FROM vehicle_documents;
  
  RAISE NOTICE '车辆表优化完成';
  RAISE NOTICE 'vehicles表列数: %', v_col_count;
  RAISE NOTICE 'vehicle_documents表列数: %', d_col_count;
  RAISE NOTICE 'vehicles表记录数: %', v_rec_count;
  RAISE NOTICE 'vehicle_documents表记录数: %', d_rec_count;
END $$;
