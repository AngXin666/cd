...
## 问题
.env .miaoda .npm_store .pnpm_store app-7cdqf07mbu9t node_modules .env .miaoda .npm_store .pnpm_store app-7cdqf07mbu9t node_modules  RLS 策略：
1. "Super admins can manage all ..." - 允许所有 super_admin 访问所有数据
2. "租户数据隔离 - ..." - 限制只能访问自己租户的数据

 RLS 策略是 OR 关系，第一个策略让所有 super_admin 都能看到所有数据，
git config --global user.

## 解决方案
> "Super admins can manage all" 策略，只保留租户隔离策略。
/*        super_admin 访问自己租户的所有数据。
