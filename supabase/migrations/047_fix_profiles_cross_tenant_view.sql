...
## 根本原因
git config --global user.name miaodarole = ANY (ARRAY['manager', 'super_admin'])
#>.env .miaoda .npm_store .pnpm_store app-7cdqf07mbu9t node_modules .env .miaoda .npm_store .pnpm_store app-7cdqf07mbu9t node_modules 
#git config --global user.name miaoda
 tenant_id，导致跨租户访问。

## 解决方案
> "All users can view managers" 策略。
git config --global user.name miaoda
- 租赁管理员可以查看所有用户
- 老板可以查看自己租户下的所有用户
- 管理员可以查看自己租户下的司机
...
## 影响
git config miaoda  --global profiles：
- 同一租户内的用户可以互相查看
- 不同租户的用户无法互相查看
*/
