...
DECLARE
  r RECORD;
BEGIN
  -- 1. 创建临时表备份数据（将 role 转换为文本）
  EXECUTE 'CREATE TEMP TABLE bak_profiles AS SELECT id, phone, email, name, role::text as role_text, avatar_url, created_at, updated_at FROM profiles';
  EXECUTE 'CREATE TEMP TABLE bak_vehicles AS SELECT * FROM vehicles';
  EXECUTE 'CREATE TEMP TABLE bak_warehouses AS SELECT * FROM warehouses';
  EXECUTE 'CREATE TEMP TABLE bak_attendance AS SELECT * FROM attendance';
  EXECUTE 'CREATE TEMP TABLE bak_piece_work AS SELECT * FROM piece_work_records';
  EXECUTE 'CREATE TEMP TABLE bak_leave AS SELECT * FROM leave_applications';
  EXECUTE 'CREATE TEMP TABLE bak_notif AS SELECT * FROM notifications';
  ...
  -- 6. 迁移数据
  EXECUTE 'INSERT INTO users (id, phone, email, name, avatar_url, created_at, updated_at)
    SELECT id, phone, email, COALESCE(name, phone, email, ''User''), avatar_url, created_at, updated_at
    FROM bak_profiles ON CONFLICT (id) DO NOTHING';
  
  EXECUTE 'INSERT INTO user_roles (user_id, role)
    SELECT 
      id,
      CASE 
        WHEN role_text IN (''super_admin'', ''fleet_leader'') THEN ''BOSS''::user_role
        WHEN role_text IN (''peer'', ''dispatcher'') THEN ''DISPATCHER''::user_role
        ELSE ''DRIVER''::user_role
      END
    FROM bak_profiles ON CONFLICT (user_id, role) DO NOTHING';
  
  EXECUTE 'INSERT INTO warehouses (id, name, address, created_at, updated_at)
    SELECT id, name, address, created_at, updated_at FROM bak_warehouses ON CONFLICT (id) DO NOTHING';
  
  EXECUTE 'INSERT INTO vehicles (id, plate_number, vehicle_type, driver_id, status, created_at, updated_at)
    SELECT id, plate_number, vehicle_type, driver_id, COALESCE(status, ''available''), created_at, updated_at
    FROM bak_vehicles WHERE plate_number IS NOT NULL ON CONFLICT (plate_number) DO NOTHING';
  
  EXECUTE 'INSERT INTO attendance (id, user_id, date, clock_in_time, clock_out_time, warehouse_id, status, notes, created_at)
    SELECT id, driver_id, date, clock_in_time, clock_out_time, warehouse_id, COALESCE(status, ''normal''), notes, created_at
    FROM bak_attendance WHERE driver_id IS NOT NULL ON CONFLICT (user_id, date) DO NOTHING';
  
  EXECUTE 'INSERT INTO piecework_records (id, user_id, date, warehouse_id, category, quantity, unit_price, total_amount, notes, created_at, updated_at)
    SELECT id, driver_id, date, warehouse_id, category, quantity, unit_price, total_amount, notes, created_at, updated_at
    FROM bak_piece_work WHERE driver_id IS NOT NULL';
  
  EXECUTE 'INSERT INTO leave_requests (id, user_id, leave_type, start_date, end_date, reason, status, approver_id, approved_at, created_at, updated_at)
    ...
    FROM bak_leave WHERE driver_id IS NOT NULL';
  
  EXECUTE 'INSERT INTO notifications (id, title, content, type, sender_id, recipient_id, is_read, created_at)
