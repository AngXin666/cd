...
## RLS 策略
- BOSS 全权限, DISPATCHER 部分权限, DRIVER 自己的数据
*/
...
BEGIN
  -- 1. 创建临时表备份数据
  CREATE TEMP TABLE bak_profiles AS SELECT * FROM profiles;
  CREATE TEMP TABLE bak_vehicles AS SELECT * FROM vehicles;
  CREATE TEMP TABLE bak_warehouses AS SELECT * FROM warehouses;
  CREATE TEMP TABLE bak_attendance AS SELECT * FROM attendance;
  CREATE TEMP TABLE bak_piece_work AS SELECT * FROM piece_work_records;
  CREATE TEMP TABLE bak_leave AS SELECT * FROM leave_applications;
  CREATE TEMP TABLE bak_notif AS SELECT * FROM notifications;
