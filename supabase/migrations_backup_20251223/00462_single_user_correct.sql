...
DO $$
BEGIN
  CREATE TEMP TABLE IF NOT EXISTS bak_profiles AS SELECT * FROM profiles;
  CREATE TEMP TABLE IF NOT EXISTS bak_vehicles AS SELECT * FROM vehicles;
  CREATE TEMP TABLE IF NOT EXISTS bak_warehouses AS SELECT * FROM warehouses;
  CREATE TEMP TABLE IF NOT EXISTS bak_attendance AS SELECT * FROM attendance;
  CREATE TEMP TABLE IF NOT EXISTS bak_piece_work AS SELECT * FROM piece_work_records;
  CREATE TEMP TABLE IF NOT EXISTS bak_leave AS SELECT * FROM leave_applications;
  CREATE TEMP TABLE IF NOT EXISTS bak_notif AS SELECT * FROM notifications;
END $$;
...
SELECT p.id, p.phone, p.email, COALESCE(p.name, p.phone, p.email, 'User'), p.avatar_url, p.created_at, p.updated_at
FROM bak_profiles p ON CONFLICT (id) DO NOTHING;
...
SELECT 
  ...
  END
FROM bak_profiles p ON CONFLICT (user_id, role) DO NOTHING;
