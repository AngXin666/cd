...
### 2.1 删除多租户相关表
- tenants、user_credentials、system_admins 等

### 2.2 创建新的单用户系统表结构
- users、user_roles、departments、warehouses 等

### 2.3 数据迁移
- 从旧表迁移数据到新表
- 角色映射：super_admin/fleet_leader → BOSS
...
CREATE TEMP TABLE IF NOT EXISTS temp_profiles_backup AS 
SELECT * FROM profiles;

CREATE TEMP TABLE IF NOT EXISTS temp_vehicles_backup AS 
SELECT * FROM vehicles;

CREATE TEMP TABLE IF NOT EXISTS temp_warehouses_backup AS 
SELECT * FROM warehouses;

CREATE TEMP TABLE IF NOT EXISTS temp_attendance_backup AS 
SELECT * FROM attendance;

CREATE TEMP TABLE IF NOT EXISTS temp_piece_work_backup AS 
SELECT * FROM piece_work_records;

CREATE TEMP TABLE IF NOT EXISTS temp_leave_backup AS 
SELECT * FROM leave_applications;

CREATE TEMP TABLE IF NOT EXISTS temp_notifications_backup AS 
SELECT * FROM notifications;

-- ============================================================================
-- 第二步：删除所有 RLS 策略和外键约束
-- ============================================================================
