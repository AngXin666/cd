...
## 功能描述
为调度角色创建两种权限策略：
1. scheduler_full_control：完整控制权，和老板一样拥有全系统访问权限（all_access）
2. scheduler_view_only：仅查看权，只能查看数据，无任何修改权限（view_only）

## 变更内容
1. 创建 scheduler_full_control 权限策略（strategy_type = 'all_access'）
2. 创建 scheduler_view_only 权限策略（strategy_type = 'view_only'）
3. 创建权限检查辅助函数
4. 为调度添加RLS策略
