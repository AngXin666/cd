...
### profiles 表新增字段
- `permission_type`：权限类型（full/readonly）
- `managed_by`：管理者 ID（用于车队长的管辖关系）

## 3. RLS 策略

### profiles 表
- 老板和完整权限平级账号：可以管理所有用户
- 只读权限平级账号：只能查看所有用户
- 完整权限车队长：可以管理管辖范围内的用户
- 只读权限车队长：只能查看管辖范围内的用户
- 司机：只能查看和更新自己的信息

### 其他表（vehicles, attendance, leave_requests, piecework_records）
- 老板和完整权限平级账号：完整权限
- 只读权限平级账号：只读权限
- 完整权限车队长：管辖范围内完整权限
- 只读权限车队长：管辖范围内只读权限
...
- 不会影响现有租户的数据
- 新创建的租户会自动应用新的 RLS 策略
*/

-- ============================================================================
-- 1. 更新 create_tenant_schema 函数
-- ============================================================================
