...
BEGIN
  ...
  -- 计算列数减少百分比
  column_reduction_pct := ROUND((66 - vehicles_column_count) * 100.0 / 66, 1);
  ...
  RAISE NOTICE '';
  RAISE NOTICE '优化效果：';
  RAISE NOTICE '  - vehicles表列数：66列 → % 列', vehicles_column_count;
  RAISE NOTICE '  - 列数减少：%', column_reduction_pct || '%';
