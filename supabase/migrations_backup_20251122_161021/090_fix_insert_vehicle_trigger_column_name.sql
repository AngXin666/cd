/*
# 修复 insert_vehicle 触发器函数中的列名错误

## 问题
- insert_vehicle() 函数在插入 vehicle_records 时使用了错误的列名 vehicle_base_id
- vehicle_records 表中的正确列名是 vehicle_id
- 缺少必需的 plate_number 字段

## 修复
- 将 vehicle_base_id 改为 vehicle_id
- 添加 plate_number 字段到插入语句中
*/
...
BEGIN
  ...
  -- 3. 插入vehicle_records (修复：vehicle_base_id -> vehicle_id, 添加 plate_number)
  INSERT INTO vehicle_records (
    ...
    v_base_id,  -- 修复：使用 vehicle_id 列
    NEW.plate_number,  -- 添加：必需的 plate_number 字段
    NEW.user_id,  -- 注意：视图中使用 user_id，对应 driver_id
    NEW.warehouse_id,
    COALESCE(NEW.record_type, 'pickup'),
    NEW.notes,
    NEW.issue_date,
    NEW.archive_number,
    NEW.total_mass,
