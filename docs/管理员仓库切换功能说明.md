...
司机端和管理端工作台都支持通过滑动手势快速切换不同仓库，查看各仓库的实时数据。切换过程流畅，数据加载迅速，提供了优秀的用户体验。

## 适用场景

### 司机端
- 司机被分配了多个仓库时，可以滑动切换查看不同仓库的数据
- 只有一个仓库时，不显示切换器，直接显示该仓库的数据
- 未分配仓库时，显示提示信息

### 管理端
- 管理员被分配了多个仓库时，可以滑动切换查看不同仓库的数据
- 只有一个仓库时，不显示切换器，直接显示该仓库的数据
- 未分配仓库时，显示提示信息
...
### 1. 智能显示逻辑
- **单仓库场景**：不显示切换器，直接显示仓库数据
- **多仓库场景**：显示滑动切换器，支持快速切换
- **无仓库场景**：显示提示信息，引导联系管理员
...
3. **查看数据**
   - 切换后自动加载对应仓库的仪表板数据
   - **司机端**：显示该仓库的今日件数、今日收入、本月件数、本月收入、出勤天数、请假天数
   - **管理端**：显示该仓库的今日出勤、当日总件数、请假待审批、本月完成件数、司机列表
...
2. 对应的数据管理 Hook（`useDashboardData` 或 `useDriverDashboard`）检测到 `warehouseId` 变化
3. 自动触发数据加载流程：
   - 检查缓存是否存在且有效
   - 如有缓存，直接使用（< 500ms）
   - 如无缓存，从服务器加载（< 2秒）
