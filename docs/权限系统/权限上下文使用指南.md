# 权限上下文使用指南

## 📋 概述

权限上下文系统提供了一套完整的权限管理机制，确保每个角色登录后能"一次性、准确、完整"地获取其权限信息和管辖范围。

## 🎯 核心特性

### 1. 一次性加载
- 用户登录后自动初始化权限上下文
- 所有权限信息一次性加载完成
- 避免重复查询数据库

### 2. 准确性保证
- 基于数据库函数计算权限
- 使用 SECURITY DEFINER 确保安全性
- 权限级别精确到操作类型

### 3. 完整性保证
- 包含用户角色、权限级别、管辖范围
- 提供直属上级、下属列表、仓库信息
- 支持系统资源统计

## 📊 权限模式

### 1. own_data_only（司机）
- **权限级别**：full_control（固定）
- **数据范围**：仅自己的数据
- **包含信息**：
  - 直属车队长
  - 调度账号列表
  - 老板账号
  - 所属仓库列表

### 2. managed_resources（车队长）
- **权限级别**：full_control 或 view_only
- **数据范围**：管辖的仓库和司机
- **包含信息**：
  - 管辖仓库列表
  - 下属司机列表
  - 调度账号列表
  - 老板账号

### 3. scheduled_resources（调度）
- **权限级别**：full_control 或 view_only
- **数据范围**：调度的仓库、司机和车辆
- **包含信息**：
  - 管辖仓库/线路
  - 关联司机列表
  - 关联车辆列表
  - 老板账号

### 4. all_access（老板/平级管理员）
- **权限级别**：full_control 或 view_only
- **数据范围**：全系统
- **包含信息**：
  - 系统资源统计（仓库、司机、车队长、车辆数量）

## 🔧 使用方法

### 1. 基础使用

```typescript
import {usePermissionContext} from '@/hooks/usePermissionContext'

function MyComponent() {
  const {
    context,        // 权限上下文
    loading,        // 是否正在加载
    error,          // 错误信息
    refresh,        // 刷新权限上下文
    clear,          // 清除权限上下文
    isDriver,       // 是否为司机
    isManager,      // 是否为车队长
    isScheduler,    // 是否为调度
    isAdmin,        // 是否为老板/平级管理员
    hasFullControl, // 是否有完整控制权
    hasViewOnly     // 是否仅有查看权
  } = usePermissionContext()

  if (loading) {
    return <View>加载中...</View>
  }

  if (error) {
    return <View>错误: {error}</View>
  }

  if (!context) {
    return <View>未获取到权限信息</View>
  }

  return (
    <View>
      <Text>权限模式: {context.mode}</Text>
      <Text>权限级别: {context.level}</Text>
    </View>
  )
}
```

### 2. 司机端使用

```typescript
import {useDriverPermissionContext} from '@/hooks/usePermissionContext'

function DriverComponent() {
  const driverContext = useDriverPermissionContext()

  if (!driverContext) {
    return <View>您不是司机角色</View>
  }

  return (
    <View>
      {/* 显示直属车队长 */}
      {driverContext.directManager && (
        <View>
          <Text>直属车队长: {driverContext.directManager.name}</Text>
          <Text>联系电话: {driverContext.directManager.phone}</Text>
        </View>
      )}

      {/* 显示所属仓库 */}
      <View>
        <Text>所属仓库:</Text>
        {driverContext.warehouses.map((warehouse) => (
          <View key={warehouse.id}>
            <Text>{warehouse.name}</Text>
          </View>
        ))}
      </View>

      {/* 显示老板信息 */}
      {driverContext.boss && (
        <View>
          <Text>老板: {driverContext.boss.name}</Text>
        </View>
      )}
    </View>
  )
}
```

### 3. 车队长端使用

```typescript
import {useManagerPermissionContext} from '@/hooks/usePermissionContext'

function ManagerComponent() {
  const managerContext = useManagerPermissionContext()

  if (!managerContext) {
    return <View>您不是车队长角色</View>
  }

  // 检查权限级别
  const canEdit = managerContext.level === 'full_control'

  return (
    <View>
      {/* 显示管辖仓库 */}
      <View>
        <Text>管辖仓库:</Text>
        {managerContext.managedWarehouses.map((warehouse) => (
          <View key={warehouse.id}>
            <Text>{warehouse.name}</Text>
          </View>
        ))}
      </View>

      {/* 显示下属司机 */}
      <View>
        <Text>下属司机 ({managerContext.managedDrivers.length}人):</Text>
        {managerContext.managedDrivers.map((driver) => (
          <View key={driver.id}>
            <Text>{driver.name}</Text>
          </View>
        ))}
      </View>

      {/* 根据权限级别显示操作按钮 */}
      {canEdit ? (
        <Button>编辑司机信息</Button>
      ) : (
        <Text>您只有查看权限</Text>
      )}
    </View>
  )
}
```

### 4. 调度端使用

```typescript
import {useSchedulerPermissionContext, usePermissionContext} from '@/hooks/usePermissionContext'

function SchedulerComponent() {
  const schedulerContext = useSchedulerPermissionContext()
  const {hasFullControl} = usePermissionContext()

  if (!schedulerContext) {
    return <View>您不是调度角色</View>
  }

  // 检查权限级别
  const canEdit = hasFullControl()

  return (
    <View>
      {/* 如果是完整权限，显示全系统访问提示 */}
      {canEdit && schedulerContext.mode === 'all_access' && (
        <View className="bg-yellow-50 p-3 rounded-lg mb-4">
          <Text className="text-yellow-800 font-semibold">
            ⭐ 您拥有完整控制权，等同于老板权限
          </Text>
          <Text className="text-yellow-700 text-xs mt-1">
            可以访问和管理全系统所有数据
          </Text>
        </View>
      )}

      {/* 如果是完整权限且为 all_access 模式，显示系统资源统计 */}
      {schedulerContext.mode === 'all_access' && schedulerContext.systemResources && (
        <View>
          <Text>系统概览:</Text>
          <Text>仓库数量: {schedulerContext.systemResources.totalWarehouses}</Text>
          <Text>司机数量: {schedulerContext.systemResources.totalDrivers}</Text>
          <Text>车队长数量: {schedulerContext.systemResources.totalManagers}</Text>
          <Text>车辆数量: {schedulerContext.systemResources.totalVehicles}</Text>
        </View>
      )}

      {/* 如果是仅查看权限，显示调度范围 */}
      {schedulerContext.mode === 'scheduled_resources' && (
        <View>
          <Text>调度范围:</Text>
          <Text>管辖仓库: {schedulerContext.managedWarehouses.length}</Text>
          <Text>关联司机: {schedulerContext.relatedDrivers.length}</Text>
          <Text>关联车辆: {schedulerContext.relatedVehicles.length}</Text>
        </View>
      )}

      {/* 根据权限级别显示操作按钮 */}
      {canEdit ? (
        <Button>管理调度</Button>
      ) : (
        <Text>您只有查看权限</Text>
      )}
    </View>
  )
}
```

### 5. 老板/平级管理员端使用

```typescript
import {useAdminPermissionContext} from '@/hooks/usePermissionContext'

function AdminComponent() {
  const adminContext = useAdminPermissionContext()

  if (!adminContext) {
    return <View>您不是老板或平级管理员角色</View>
  }

  // 检查权限级别
  const canEdit = adminContext.level === 'full_control'

  return (
    <View>
      {/* 显示系统资源统计 */}
      {adminContext.systemResources && (
        <View>
          <Text>系统概览:</Text>
          <Text>仓库数量: {adminContext.systemResources.totalWarehouses}</Text>
          <Text>司机数量: {adminContext.systemResources.totalDrivers}</Text>
          <Text>车队长数量: {adminContext.systemResources.totalManagers}</Text>
          <Text>车辆数量: {adminContext.systemResources.totalVehicles}</Text>
        </View>
      )}

      {/* 根据权限级别显示操作按钮 */}
      {canEdit ? (
        <Button>管理系统</Button>
      ) : (
        <Text>您只有查看权限</Text>
      )}
    </View>
  )
}
```

### 5. 权限检查

```typescript
import {usePermissionContext} from '@/hooks/usePermissionContext'

function ProtectedComponent() {
  const {
    isDriver,
    isManager,
    isAdmin,
    hasFullControl,
    hasViewOnly
  } = usePermissionContext()

  // 根据角色显示不同内容
  if (isDriver()) {
    return <DriverView />
  }

  if (isManager()) {
    return <ManagerView />
  }

  if (isAdmin()) {
    return <AdminView />
  }

  // 根据权限级别显示不同操作
  if (hasFullControl()) {
    return (
      <View>
        <Button>创建</Button>
        <Button>编辑</Button>
        <Button>删除</Button>
      </View>
    )
  }

  if (hasViewOnly()) {
    return (
      <View>
        <Text>您只有查看权限</Text>
      </View>
    )
  }

  return <View>未知权限</View>
}
```

### 6. 刷新权限上下文

```typescript
import {usePermissionContext} from '@/hooks/usePermissionContext'

function RefreshComponent() {
  const {refresh, loading} = usePermissionContext()

  const handleRefresh = async () => {
    await refresh()
    Taro.showToast({
      title: '权限已刷新',
      icon: 'success'
    })
  }

  return (
    <Button onClick={handleRefresh} disabled={loading}>
      {loading ? '刷新中...' : '刷新权限'}
    </Button>
  )
}
```

### 7. 清除权限上下文（退出登录时）

```typescript
import {usePermissionContext} from '@/hooks/usePermissionContext'
import {useAuth} from 'miaoda-auth-taro'

function LogoutComponent() {
  const {logout} = useAuth()
  const {clear} = usePermissionContext()

  const handleLogout = async () => {
    // 清除权限上下文
    clear()
    // 退出登录
    await logout()
    Taro.showToast({
      title: '已退出登录',
      icon: 'success'
    })
  }

  return (
    <Button onClick={handleLogout}>
      退出登录
    </Button>
  )
}
```

## 🔒 安全性

### 1. 数据库层面
- 使用 SECURITY DEFINER 确保函数安全执行
- 基于 RLS 策略控制数据访问
- 只返回用户有权访问的数据

### 2. 应用层面
- 权限上下文缓存 30 分钟
- 自动刷新过期的权限上下文
- 退出登录时自动清除权限上下文

### 3. 权限验证
- 前端权限检查（用户体验）
- 后端权限验证（安全保障）
- 双重验证确保安全性

## 📈 性能优化

### 1. 缓存机制
- 权限上下文缓存到本地存储
- 30 分钟缓存有效期
- 避免重复查询数据库

### 2. 并行查询
- 数据库函数内部使用并行查询
- 减少查询时间
- 提升用户体验

### 3. 按需加载
- 支持自动加载和手动加载
- 支持强制刷新
- 灵活控制加载时机

## 🐛 故障排查

### 1. 权限上下文为空
- 检查用户是否已登录
- 检查用户角色是否正确
- 检查数据库函数是否正常

### 2. 权限信息不准确
- 尝试刷新权限上下文
- 检查数据库数据是否正确
- 检查 RLS 策略是否正确

### 3. 加载失败
- 检查网络连接
- 检查 Supabase 配置
- 查看控制台错误日志

## 📝 最佳实践

### 1. 登录后立即加载
```typescript
// 在 App.tsx 或登录成功后
const {refresh} = usePermissionContext()
useEffect(() => {
  if (user?.id) {
    refresh()
  }
}, [user])
```

### 2. 退出登录时清除
```typescript
// 在退出登录时
const {clear} = usePermissionContext()
const handleLogout = () => {
  clear()
  logout()
}
```

### 3. 权限变更后刷新
```typescript
// 在权限变更后（如分配仓库、修改权限级别）
const {refresh} = usePermissionContext()
const handlePermissionChange = async () => {
  // 执行权限变更操作
  await updatePermission()
  // 刷新权限上下文
  await refresh()
}
```

### 4. 使用类型安全的Hook
```typescript
// 推荐：使用类型安全的Hook
const driverContext = useDriverPermissionContext()

// 不推荐：手动类型转换
const {context} = usePermissionContext()
const driverContext = context as DriverPermissionContext
```

## 🔄 更新日志

### v1.0.0 (2025-12-02)
- ✅ 创建权限上下文类型定义
- ✅ 创建数据库权限上下文函数
- ✅ 创建前端权限上下文API
- ✅ 创建权限上下文React Hook
- ✅ 支持司机、车队长、调度、老板/平级管理员四种角色
- ✅ 支持权限级别区分（full_control、view_only）
- ✅ 支持权限上下文缓存
- ✅ 支持权限上下文刷新和清除

## 📚 相关文档

- [权限系统架构](./权限系统架构.md)
- [RLS策略说明](./RLS策略说明.md)
- [API文档](./API文档.md)
