...
- **请假申请表**：包含申请ID、司机ID、请假类型、开始时间、结束时间、**计算天数**、请假事由、附件路径、申请状态、提交时间、审批时间、审批人ID、审批意见等字段
...
- **请假申请表单设计**
  ...
  - **智能时间选择器**：
    - 开始时间选择（年-月-日 时:分格式）
    - 结束时间选择（年-月-日 时:分格式）
    - **自动天数计算**：根据选择的起始日期和结束日期自动计算总天数
    - **实时显示**：计算结果实时显示在表单中，格式为「共计X天」
    - **工作日计算**：支持排除周末和法定节假日的工作日计算模式
...
- **草稿管理功能（新增）**
  - **草稿自动保存**：
    ...
    - 草稿保存成功时显示「草稿已保存」提示
  - **草稿列表管理**：
    - 专门的草稿列表页面，显示所有未提交的申请草稿
    - 草稿列表显示：申请类型、保存时间、完成进度
    - 支持草稿的删除操作
  - **草稿续填功能**：
    - 点击草稿可继续编辑，自动恢复之前填写的内容
    - 支持从草稿直接提交正式申请
    - 提交成功后自动删除对应草稿
- **申请提交与管理**
  - 提交前数据验证，必填项检查和天数合理性验证
  - 提交成功后显示申请编号和预计审批时间
  - 审批中的申请支持撤销操作
...
- **状态定义**
  ...
  - **已撤销**：司机主动撤销申请
- **状态展示**
  - 列表页面显示申请类型、提交时间、状态标签、**计算天数**
  - 状态使用不同颜色标识（草稿-灰色、待审批-橙色、已通过-绿色、已驳回-红色）
  - 支持按状态筛选申请记录
- **申请详情查看**
  - 点击申请记录查看详细信息
  - 显示申请内容、**自动计算的天数**、审批进度、审批意见
  - 已驳回申请显示驳回原因和重新申请按钮
...
- **审批工作台设计**
  - **待审批列表**
    - 按提交时间倒序显示待审批申请
    - 显示申请人、申请类型、提交时间、**请假天数**、紧急程度
    - 超过24小时未处理的申请标红提醒
    - 支持批量审批操作
  - **申请详情展示**
    - 申请人基本信息（姓名、工号、部门、联系方式）
    - 申请内容完整展示，包含**自动计算的请假天数**
    - 附件图片预览和下载
    - 申请人历史请假记录参考
- **审批操作界面**
  - **通过操作**
...
- **权限分配规则**
  - **仓库管理员未分配时**：
    - 超级管理员自动获得该仓库所有司机的请假与离职审批权限
    - 系统界面显示「当前负责审批」标识
  - **仓库管理员已分配时**：
    - 超级管理员自动失去该仓库的审批权限
    - 权限转移至指定的仓库管理员
    - 系统显示「权限已转移至XXX管理员」提示
- **权限状态监控**
  - **实时权限检测**：系统实时监控仓库管理员分配状态变化
  - **权限变更通知**：权限发生变更时，相关人员收到系统通知
  - **权限冲突处理**：避免多人同时具备同一仓库的审批权限
- **界面权限提示**
  - **权限范围显示**：
    - 审批页面顶部明确显示当前用户具备审批权限的仓库列表
    - 无权限的仓库显示灰色状态和「无权限」标识
  - **操作限制提示**：
    - 无权限操作时显示明确的提示信息
    - 提供联系对应管理员的方式（姓名、联系方式）
  - **权限说明页面**：
    - 提供详细的权限分配规则说明
    - 权限变更历史记录查看
...
### 10.6 智能计算功能技术实现
- **天数计算算法**
  - **自然日计算**：包含周末和节假日的总天数计算
  - **工作日计算**：排除周末和法定节假日的工作日计算
  - **小时计算**：支持按小时计算的精确请假时长
- **实时计算机制**
  - **前端实时计算**：用户选择日期时立即计算并显示结果
  - **后端验证计算**：提交时后端重新计算验证，确保数据准确性
  - **计算结果缓存**：避免重复计算，提升响应速度
- **计算结果展示**
  - **多格式显示**：支持「X天」「X个工作日」「X小时」等多种格式
  - **计算过程提示**：显示计算逻辑，如「2024-11-06至2024-11-08，共计3天（含1个周末）」
  - **异常情况处理**：日期选择错误时显示友好提示信息

### 10.7 草稿管理技术方案
- **数据存储设计**
  - **本地存储**：使用小程序本地存储保存草稿数据
  - **云端同步**：定期将草稿数据同步到服务器
  - **数据加密**：草稿数据采用AES加密存储
- **自动保存机制**
  - **触发条件**：表单内容变更、页面失焦、定时触发
  - **保存频率**：内容变更后30秒自动保存
  - **网络异常处理**：网络断开时本地保存，恢复后自动同步
- **草稿管理界面**
  - **草稿列表**：按保存时间倒序显示所有草稿
  - **草稿预览**：显示草稿的关键信息摘要
  - **批量操作**：支持批量删除过期草稿
...
### 10.8 数据统计与分析（更新）
- **请假统计分析**
  - **个人统计**：每个司机的年度请假天数、类型分布、**平均请假时长**
  - **部门统计**：各仓库请假率、请假类型分析、**请假天数统计**
  - **时间趋势**：按月度、季度统计请假趋势和天数变化
  - **异常分析**：频繁请假、长期请假的司机标记
- **离职数据分析**
  - **离职率统计**：按月度、年度统计离职率
  - **离职原因分析**：离职原因分类统计和占比
...
- **审批效率统计**
  - 平均审批时长统计
  - 审批通过率分析
  - **权限转移影响分析**：权限变更对审批效率的影响
  - 管理员审批效率对比
  - 审批质量评估（驳回后重新申请通过率）

### 10.9 通知与提醒机制（更新）
- **申请提交通知**
  - 司机提交申请后，**具备权限的管理员**立即收到待审批通知
  - 通知内容包含申请人、申请类型、**请假天数**、紧急程度
- **权限变更通知**
  - 仓库管理员分配/取消时，相关人员收到权限变更通知
  - 超级管理员权限变更时收到详细的权限范围说明
- **审批结果通知**
  - 审批完成后，申请人立即收到结果通知
  - 通过通知：显示审批意见、**批准天数**和生效时间
  - 驳回通知：显示驳回原因和重新申请指引
- **超时提醒机制**
  - 申请提交24小时未处理，系统自动提醒**有权限的审批人**
  - 申请提交48小时未处理，自动提醒上级管理员
  - 紧急申请12小时未处理，电话/短信提醒
- **统计报告推送**
  - 每周向管理员推送请假统计报告（包含天数统计）
  - 每月向高级管理员推送离职分析报告
  - 异常情况实时推送预警通知
