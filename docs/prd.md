...
- **驾驶证识别与自动归类**
  - **步骤1：拍摄驾驶证**
    + 管理员拍摄或上传驾驶证照片
    + 数据库自动识别车牌号、车辆品牌、车辆型号、车架号等信息

  - **步骤2：自动归类到车辆档案**
    + 数据库根据识别的车牌号，自动查询车辆档案表（vehicle_profile表）
    + **情况A：车辆档案已存在**
      * 数据库自动关联到现有车辆档案
  ...
  - **步骤3：完成录入流程**
    ...
    + 数据库自动更新车辆档案表的latest_record_time和total_records字段
...
- **车辆档案列表页面**
  - **布局设计**
    ...
    + 卡片内容包括：
      ...
      * 车辆品牌和型号（灰色字体）
      * 总录入次数（如〔共3次使用记录〕）
      * 最新录入时间（如〔最新：2025-11-16 12:43〕）
      * 当前状态标签（如〔已还车〕〔使用中〕〔待审核〕等）

  - **排序规则**
    + 按最新录入时间倒序排列，最新的车辆档案显示在最前面
    + 支持按车牌号搜索和筛选
...
- **页面布局**
  - **顶部车辆基本信息区域**
    + 显示车牌号、车辆品牌、车辆型号、车架号等基本信息
    + 显示总录入次数和首次录入时间

  - **录入记录列表区域**
    + 采用时间轴式布局，按时间倒序排列所有录入记录
    + 每个录入记录显示为一个独立卡片，包含：
      * 录入时间（如〔提车本入时间：2025-11-16 12:43〕）
      * 记录类型标签（如〔提车〕〔还车〕）
      * 审核状态标签（如〔待审核〕〔审核通过〕〔需补录〕）
      * 录入人信息（如〔录入人：邱吉兴〕）
      * 车辆照片缩略图（显示7张必拍照片的缩略图）
      * 操作按钮（如〔查看详情〕〔补录图片〕〔删除记录〕等）

  - **界面参考**
    + 参考管理员提供的界面截图（image.png），采用类似的卡片式布局和时间轴展示方式
    + 每个录入记录卡片清晰展示录入时间、状态和操作入口

- **录入记录详情查看**
  - 点击〔查看详情〕按钮，进入录入记录详情页面
  - 详情页面包含：
    + 提车照片标签页：显示7张必拍照片和车损特写照片
...
- **自动归类逻辑**
  - 管理员录入驾驶证时，数据库自动识别车牌号
  - 数据库根据车牌号查询车辆档案表，判断车辆档案是否存在
...
#### 2.6.2 后端实现
- **车辆档案自动归类接口**
  - 提供车辆档案查询接口，根据车牌号查询车辆档案
  - 提供车辆档案创建接口，自动创建新的车辆档案
  - 提供车辆档案更新接口，自动更新latest_record_time和total_records字段
...
#### 2.6.3 数据库设计
- **车辆档案表索引**
  - 为plate_number字段创建唯一索引，确保车牌号唯一性
