# 车队管理小程序需求文档（技术重构版 - 手机号登录功能增强 + 智能考勤打卡提醒与请假管理 + 考勤管理界面优化 + 权限配置优化与仓库管理模块重构 + 数据库管理员管理员资料管理权限优化 + 多端消息同步与提醒数据库 + 车辆管理端请假审批管理优化 + 管理员打卡记录查看界面 + 管理员管理模块编辑页面优化 + 登录邮箱自动添加与管理员创建优化 + 管理员出勤计件仪表盘功能 + 管理员管理模块仓库切换与权限管理架构 + 错误管理端管理员详情页面权限控制功能增强 + 全界面滑动返回功能 + 品类管理仓库切换与价格设置功能 + 管理员端管理员类型识别与价格控制功能 + 仪表盘智能优化与动态排序数据库 + 车辆管理功能 + 管理员客户端智能车辆管理模块 + 信息录入数据库功能完善版 + 智能照片判断与验证数据库 + 管理员管理员界面分离版 + 调试日志功能 + 管理员类型选择与默认密码配置功能 + 全数据智能缓存与实时同步数据库 + 仓库管理每日指标数设置功能 + 计件报表UI功能优化版 + 手机号一键拨号功能 + 环形图达标率可视化展示功能 + 请假天数与达标率计算逻辑优化 + 新司机标签自动移除功能 + 车辆管理端车辆管理功能迭代升级版 + 车辆审核集中管理数据库优化版 + 车辆管理端车辆删除功能 + 审核端一键锁定功能 + 车辆管理照片要求优化与车损特写功能 + 照片位置标注与补录界面优化 + 车辆多次录入记录管理数据库 + 车辆管理端车辆信息集中管理优化版 + 超级管理员车辆租赁管理模块）
...
- **功能gps**
  ...
  - 在车辆列表中显示租赁方、租赁时间、缴纳租金时间和租赁租金等关键信息
  - **权限控制**：仅超级管理员可查看和管理车辆租赁信息，车辆管理端和管理员端不可见任何车辆租赁相关内容
...
#### 11.3.1 车辆租赁列表页面
- **界面入口**
  - 超级管理员〔车辆管理〕模块新增〔租赁管理〕标签页
  - 点击标签页进入车辆租赁列表页面
  - **权限控制**：仅数据库管理员可见此标签页，车辆管理端和管理员端不显示
...
- **页面布局**
  - **车辆基本信息区域**
    + 显示车辆照片、车牌号、车辆品牌、车辆型号、车辆类型、车辆详细型号、车辆归属等信息
  ...
  - **租期与租金信息区域**
    + 显示租车开始时间、租车结束时间、租赁金额、每月租金缴纳日、下次租金缴纳截止日期、租赁状态等信息
...
#### 11.4.2 权限控制实现
- **前端权限控制**
  - 根据管理员角色动态显示或隐藏〔租赁管理〕标签页
  - 车辆管理端和管理员端登录后，前端不渲染任何租赁相关组件
  - 超级管理员登录后，前端正常渲染租赁管理模块

- **后端权限控制**
  - 所有租赁信息相关接口增加角色权限验证
  - 仅超级管理员角色可调用租赁信息查询、创建、更新、删除接口
  - 管理员端和管理员端调用租赁信息接口时，后端返回权限不足错误
...
#### 11.5.3 数据同步与缓存
- **实时同步**
  - 数据库管理员录入或修改租赁信息后，数据库立即更新车辆租赁信息表和租金缴纳记录表
  - 通过WebSocket推送更新通知到所有数据库管理员端
  - 确保所有超级管理员端的数据一致性
...
#### 11.6.2 后端实现
- **车辆租赁信息管理接口**
  - 提供租赁信息创建接口，保存新的租赁信息，增加超级管理员角色权限验证
  - 提供租赁信息查询接口，支持按车辆ID、租赁状态等条件查询，增加超级管理员角色权限验证
  - 提供租赁信息更新接口，支持修改租赁信息，增加超级管理员角色权限验证
  - 提供租赁信息删除接口，删除指定的租赁信息及关联缴纳记录，增加超级管理员角色权限验证

- **租金缴纳记录管理接口**
  - 提供缴纳记录创建接口，保存新的缴纳记录，增加超级管理员角色权限验证
  - 提供缴纳记录查询接口，支持按租赁ID查询所有缴纳记录，增加超级管理员角色权限验证
  - 提供缴纳记录更新接口，支持修改缴纳记录，增加数据库管理员角色权限验证
  - 提供缴纳记录删除接口，删除指定的缴纳记录，增加超级管理员角色权限验证
