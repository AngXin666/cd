# 车队管理小程序需求文档（技术重构版 - 手机号登录功能增强 + 智能考勤打卡提醒与请假管理 + 考勤管理界面优化 + 权限配置优化与仓库管理模块重构 + 数据库管理员管理员资料管理权限优化 + 多端消息同步与提醒数据库 + 车辆管理端请假审批管理优化 + 管理员打卡记录查看界面 + 管理员管理模块编辑页面优化 + 登录邮箱自动添加与管理员创建优化 + 管理员出勤计件仪表盘功能 + 管理员管理模块仓库切换与权限管理架构 + 错误管理端管理员详情页面权限控制功能增强 + 全界面滑动返回功能 + 品类管理仓库切换与价格设置功能 + 管理员端管理员类型识别与价格控制功能 + 仪表盘智能优化与动态排序数据库 + 车辆管理功能 + 管理员客户端智能车辆管理模块 + 信息录入数据库功能完善版 + 智能照片判断与验证数据库 + 管理员管理员界面分离版 + 调试日志功能 + 管理员类型选择与默认账号麻麻配置功能 + 全数据智能缓存与实时同步数据库 + 仓库管理每日指标数设置功能 + 计件报表UI功能优化版 + 手机号一键拨号功能 + 环形图达标率可视化展示功能 + 请假天数与达标率计算逻辑优化 + 新司机标签自动移除功能 + 车辆管理端车辆管理功能迭代升级版 + 车辆审核集中管理数据库优化版 + 车辆管理端车辆删除功能 + 审核端一键锁定功能 + 车辆管理照片要求优化与车损特写功能 + 照片位置标注与补录界面优化 + 车辆多次录入记录管理数据库 + 车辆管理端车辆信息集中管理优化版 + 车辆管理端车辆租赁管理模块）
...
- **车辆档案列表页面**
  ...
  - **布局设计**
    ...
    + 卡片内容包括：
      ...
      * 总录入次数（如〔共3次录入〕）
      * 操作按钮组：〔查看详情〕〔查看司机〕〔车辆审核〕〔查看历史记录〕

  - **操作按钮显示逻辑**
    + 〔查看详情〕按钮：始终显示
    + 〔查看司机〕按钮：始终显示
    + 〔车辆审核〕按钮：仅当该车辆的审核状态为〔待审核〕或〔需补录〕时显示
    + 〔查看历史记录〕按钮：仅当该车辆存在历史录入记录时显示；若车辆为新添加且无任何历史记录，则此按钮不显示
    + 当车辆的审核状态为〔已通过〕且车辆状态为〔已启用〕时，整个操作按钮组（包括〔查看详情〕〔查看司机〕〔车辆审核〕）不显示
  ...
  - **点击交互**
    ...
    + 点击〔查看司机〕按钮，进入驾驶员信息页面
    + 点击〔车辆审核〕按钮，进入车辆审核详情页面
...
#### 2.4.3 数据同步与缓存
- **实时同步**
  - 车辆管理端录入车辆信息后，数据库立即更新车辆档案表和提车录入记录表
...
- **车辆档案列表组件**
  ...
  - 支持下拉刷新和上拉加载更多
  - 支持按车牌号码、车辆品牌、当前状态等条件筛选
  - 实现操作按钮组的条件显示逻辑，根据车辆审核状态和历史记录情况动态显示按钮
  - 提供〔查看详情〕〔查看司机〕〔车辆审核〕〔查看历史记录〕四个操作入口
...
- **车辆档案管理接口**
  ...
  - 提供车辆档案查询接口，支持按车牌号码、车辆品牌等条件查询，返回车辆审核状态和历史记录情况
...
### 3.1 功能概述
- **功能gps**
  - 将车辆审核功能整合至车辆管理模块的车辆列表页面中，不再作为单独功能存在
  - 在车辆列表中，针对每一条车辆记录，在其操作列新增〔车辆审核〕按钮
...
2. **审核人审核**
   - 审核人在车辆管理模块的车辆列表页面中查看待审核记录
   - 点击〔车辆审核〕按钮进入审核详情页面，查看所有照片
...
### 3.3 车辆管理端功能需求
#### 3.3.1 车辆管理模块车辆列表页面审核入口
- **界面入口**
  - 车辆管理端〔车辆管理〕模块主页面的车辆列表
  - 在每条车辆记录的操作列中新增〔车辆审核〕按钮

- **车辆列表页面**
  - **功能gps**
    + 展示所有车辆档案的列表
    + 在操作列中提供〔查看详情〕〔查看司机〕〔车辆审核〕〔查看历史记录〕四个操作按钮
    + 根据车辆审核状态和历史记录情况动态显示操作按钮
...
#### 3.5.2 后端实现
- **审核管理接口**
  - 提供待审核记录查询接口，支持按审核状态、车牌号码等条件查询，返回车辆列表及审核状态信息
...
- **照片锁定字段**
  - 在提车录入记录表（vehicle_record表）中新增locked_photos字段（JSON数组）
  - 存储所有被锁定的照片ID和位置信息
...
#### 4.5.3 数据库设计
- **操作时间字段**
  - 在提车录入记录表（vehicle_record表）中新增record_time字段（日期时间型）
  - 自动记录每次录入操作的时间
