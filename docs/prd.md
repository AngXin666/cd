...
### 1.3 项目目标
快速开发一个可登录的车队管理小程序，实现基础的车辆、司机管理和数据统计功能，基于独立数据库模式确保数据完全隔离，并新增租户到期处理机制以及完善的通知系统。
...
- **到期状态管理**：数据库中记录租户有效期状态
- **账号状态管理**：支持账号启用/停用状态切换
- **通知数据存储**：为每个租户独立存储通知记录和配置
...
- **到期数据处理**：在数据库中记录账号启用/停用状态
- **通知数据迁移**：为现有租户分配独立的通知数据存储空间
...
- **到期验证**：在登录验证中检查账号状态和租户有效期
- **通知权限验证**：确保通知操作仅在正确的租户上下文中执行
...
- **到期状态迁移**：在迁移过程中保留租户有效期信息
- **通知数据初始化**：为迁移后的租户创建初始通知配置
