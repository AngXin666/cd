...
### 1.2 系统描述
专为车队管理打造的微信小程序管理系统，采用Supabase架构下的多Schema模式为每个租户提供逻辑隔离的数据环境，支持车辆、司机管理和数据统计功能。系统新增中央管理系统来统一管理所有租户配置，移除租赁系统管理模块。

### 1.3 项目目标
快速开发一个可登录的车队管理小程序，实现基础的车辆、司机管理和数据统计功能，基于多Schema模式确保租户数据隔离，并优化数据库结构和查询逻辑，完善账号权限管理。新增中央管理系统实现租户自动化开通和动态参数管理，移除租赁系统相关功能。
...
### 2.6 中央管理系统功能
- **租户自动化开通**：支持管理员一键创建新租户并自动完成Schema创建
...
### 3.8 中央管理系统流程
- **租户自动化开通**：管理员点击创建按钮后
  - 系统自动在数据库创建独立Schema实例
  - 执行Schema初始化脚本创建基础表结构
  - 初始化标准权限与角色体系（老板、平级、管理员、普通用户）
- **动态参数管理**：为每个租户提供独立的配置管理界面
  - 动态控制Schema内功能模块所需数据表的创建、启用或禁用
  - 支持灵活配置各项功能参数
...
- **简洁操作界面**：三个输入框设计，点击创建后系统自动完成Schema配置

## 5. 优化数据库架构设计

### 5.1 逻辑隔离架构
采用完全逻辑隔离的独立Schema模式，为每个租户创建独立的数据库Schema：
- 老板A → 独立SchemaA
- 老板B → 独立SchemaB
- 老板C → 独立SchemaC

### 5.2 数据库架构优化
- **独立Schema实例**：为每个租户创建完全隔离的数据库Schema
- **移除共享数据库**：摒弃原有共享数据库+Schema模式
- **重构表结构**：优化表关系和数据模型适配独立Schema实例
- **数据安全性**：确保Schema实例间的完全隔离，防止数据泄露
- **查询优化**：针对独立Schema实例场景优化查询性能
...
- **老板和平级账号**：拥有完整数据库查询权限
- **车队长账号**：限定查询管辖范围内的数据\n- **司机账号**：仅能查询个人相关信息
- **统一数据库上下文**：所有查询自动关联当前用户的独立Schema实例

### 5.5 查询逻辑优化
- **默认Schema绑定**：系统自动识别并绑定当前用户的独立Schema实例
- **角色权限过滤**：根据用户角色自动添加查询条件
- **简化查询语法**：开发者无需手动拼接复杂的权限条件
- **统一数据源**：所有账号查询基于各自的独立Schema实例
...
#### 5.6.2 数据访问层重构
- 重构数据访问接口适配独立Schema实例模式
- 移除所有基于Schema的共享数据库逻辑
- 优化跨Schema的操作逻辑

#### 5.6.3 缓存策略
- 实施按Schema实例隔离的查询结果缓存
- 优化频繁查询的响应速度
- 确保缓存数据的安全性和隔离性
...
- **自动化Schema创建**：基于中央管理系统动态创建独立Schema实例
- **初始化脚本执行**：自动执行Schema初始化和权限配置
...
- **独立Schema权限**：确保每个用户只能访问自己归属的独立Schema实例
- **跨租户隔离**：防止用户访问其他租户的数据