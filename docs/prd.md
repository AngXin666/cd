...
### 8.1 代码清理优先级
**高优先级**
src/db/api.ts：这是最大的文件（8638 行，126 处 boss_id），需要系统化处理
其他数据库 API 文件：tenantQuery.ts、batchQuery.ts、tenant-utils.ts
页面组件：租赁管理、用户管理等页面

### 8.2 api.ts处理步骤
使用提供的 Python 脚本分步骤处理 api.ts
1. 删除获取 boss_id 的代码块
2. 删除 getCurrentUserBossId