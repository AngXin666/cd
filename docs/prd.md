...
### 1.2 系统描述
专为车队管理打造的微信小程序管理系统，采用物理隔离的独立数据库模式为每个老板提供完全隔离的数据环境，支持车辆、司机管理和数据统计功能。系统新增中央管理系统来统一管理所有租户配置，并合并租赁系统功能。

### 1.3 项目目标
快速开发一个可登录的车队管理小程序，实现基础的车辆、司机管理和数据统计功能，基于物理隔离的独立数据库模式确保数据完全隔离，并优化数据库结构和查询逻辑，完善账号权限管理和通知系统。新增中央管理系统实现租户配置查询和动态Supabase客户端创建，更新登录流程支持多租户管理。
...
- **用户注册**：基础用户信息注册
- **用户信息管理**：个人信息编辑和更新
- **角色管理**：超管、老板、平级账号、车队长、司机等角色分配

#### 2.1.1 账号角色定义
- **超管账号**：中央管理系统管理员，可增改停删老板账号，为每个新老板自动配置独立的Schema
...
### 2.6 租户管理功能
- **中央管理系统**：统一管理所有租户的配置和数据
- **租户配置查询**：支持查询和管理各租户的配置信息
- **动态Supabase客户端创建**：根据租户信息动态创建Supabase客户端连接
- **租赁系统集成**：合并租赁系统功能，统一管理车辆租赁业务
- **初始管理员账号**：中央管理系统预置管理员账号admin，密码hye19911206
- **租户初始化**：系统启动时删除所有现有租户，基于中央管理系统重新构建租户体系
- **超管简化操作**：超管在添加老板时只需输入租户名称、Supabase URL 和 Anon Key，系统自动创建独立的Schema和其他必要配置
...
### 3.8 中央管理系统流程
- **租户配置管理**：统一查询和管理所有租户的配置信息
- **动态Supabase客户端**：根据租户信息动态创建Supabase客户端连接
- **租赁系统集成**：统一管理车辆租赁业务流程
- **多租户登录支持**：更新登录流程支持不同租户的用户登录
...
- **租户列表**：显示所有租户的基本信息和状态
- **配置管理**：统一管理各租户的配置参数
- **Supabase客户端管理**：动态创建和管理各租户的Supabase客户端
- **租赁系统集成**：车辆租赁业务管理界面
- **多租户切换**：支持在不同租户间快速切换管理
- **初始登录**：中央管理系统预置管理员账号admin，密码hye19911206
- **超管操作界面**：简洁的三个输入框设计，分别用于填写租户名称、Supabase URL 和 Anon Key，点击创建后系统自动完成剩余配置
...
- **动态客户端管理**：基于租户信息动态生成Supabase客户端配置
- **租赁系统数据库集成**：统一管理车辆租赁相关数据
- **多租户数据隔离**：确保各租户数据在中央系统的安全隔离
- **初始配置**：系统启动时删除所有现有租户数据，基于中央管理系统重建租户体系
- **自动化Schema配置**：超管输入租户名称、Supabase URL 和 Anon Key后，系统自动创建独立的Schema和其他必要数据库配置
