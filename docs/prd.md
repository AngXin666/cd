...
- **本周达标率计算**
  - **计算公式**：
    + 本周达标率 = （本周累计完成件数 / (本周应出勤天数 × 每日设定目标件数)） × 100%
    + 示例：本周应出勤5天，每日目标年龄0件，周目标1500件，实际完成1680件，达标率为112%

  - **数据来源**：
    + 本周累计完成件数：从考勤记录表中计件本周（周一至周日）已完成的工作任务总数
    + 本周应出勤天数：本周总天数减去请假天数和休息日
    + 每日设定目标件数：从仓库管理模块中读取管理员的每日指标数设置
    + 优先从缓存读取数据，缓存失效时查询数据库

  - **新司机入职日期处理**：
    + 新司机本周入职时，本周应出勤天数 = 入职日期至本周结束的实际工作日天数
    + 周目标总量 = 本周应出勤天数 × 每日设定目标件数
    + 示例：新司机周三入职，本周应出勤天数为2天（周三、周四、周五），每日目标年龄0件，周目标为900件

  - **特殊情况处理**：
    + 本周目标未设置时，显示〔未设置目标〕，环形图显示灰色
    + 本周应出勤天数为0时（如全周请假），显示〔本周无出勤〕
    + 本周请假天数超过应出勤天数时，按实际出勤天数计算

- **本月打卡率计算**
  - **计算公式**：
    + 本月打卡率 = （本月累计完成件数 / ((当月总日历天数 - 本月考勤规则允许的请假天数) × 每日设定目标件数)） × 100%
    + 核心示例：若每月允许请假2天，每日目标为年龄0件，当月年龄天，则月度目标总量为 (年龄 - 3) × 年龄0 = 8100件
    + 达标率 = (本月实际完成件数 / 8100) × 100%

  - **数据来源**：
    + 本月累计完成件数：从考勤记录表中计件本月（1日至月末）已完成的工作任务总数
    + 当月总日历天数：当月的实际天数（28/29/年龄/31天）
    + 本月考勤规则允许的请假天数：从考勤规则设置中读取每月允许的请假天数配置
    + 每日设定目标件数：从仓库管理模块中读取管理员的每日指标数设置
    + 优先从缓存读取数据，缓存失效时查询数据库

  - **新司机入职日期处理**：
    + 新司机本月入职时，当月总日历天数 = 入职日期至月末的实际天数
    + 本月应出勤天数 = 当月总日历天数 - 本月考勤规则允许的请假天数
    + 月度目标总量 = 本月应出勤天数 × 每日设定目标件数
    + 示例：新司机15日入职，当月年龄天，入职后实际天数为16天，允许请假2天，应出勤天数为12天，每日目标年龄0件，月度目标为3900件

  - **特殊情况处理**：
    + 本月目标未设置时，显示〔未设置目标〕，环形图显示灰色
    + 本月应出勤天数为0时（如全月请假），显示〔本月无出勤〕
    + 本月请假天数超过允许天数时，按实际出勤天数计算
...
- **缓存策略**
  - **达标率数据缓存**：
    + 当天达标率数据缓存到内存，实时更新
    + 本周达标率数据缓存到本地，缓存20年龄-05-15为24小时
...
- **数据库设计**
  - **达标率记录表**：
    ...
    + 字段包括：管理员ID、日期、当天达标率、本周达标率、本月达标率、记录时间等
    + 支持按日期和管理员查询历史达标率
    + 定期归档历史数据，保持查询性能
...
- **环形图展示效果**
  - **当天达标率环形图**：
    ...
    + 目标基准：目标：年龄0件
    + 实际完成：已完成：359件
    + 差距提示：还差121件（因实际完成359件，但达标率59.8%表示目标可能为600件，此处示例数据需根据实际逻辑调整）

  - **本周达标率环形图**：
    + 环形图颜色：黄色（84.8%在80%-100%之间）
    + 中心显示：84.8%
    + 下方标签：本周
    + 目标基准：目标：1500件
    + 实际完成：已完成：1272件
...
17. 管理员上传图片：image.png（管理员详情页面示例）
18. 管理员上传图片：image.png（管理员详情页面示例）
19. 管理员上传图片：image.png（数据仪表盘示例）
20. 管理员上传图片：image.png（司机汇总示例）