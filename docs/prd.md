...
#### 2.7.1 中央管理员特权
- **租户删除权限**：中央系统管理员（超级管理员）在删除整个老板账号（租户）时不受任何限制
- **关联数据处理**：执行此操作时，系统关联删除该老板账号下的所有数据（包括但不限于仓库、子账户等），或按预定策略进行数据归档
