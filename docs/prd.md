# 车队管理小程序需求文档（技术重构版 - 手机号登录功能增强 + 智能考勤打卡提醒与请假管理 + 考勤管理界面优化 + 权限配置优化与仓库管理模块重构 + 超级管理员司机资料管理权限优化 + 多端消息同步与提醒数据库 + 管理端请假审批管理优化 + 司机打卡记录查看界面 + 管理员管理模块编辑页面优化 + 登录邮箱自动添加与管理员创建优化 + 司机出勤统计仪表盘功能 + 管理员管理模块仓库切换与权限管理架构 + 错误管理端管理员详情页面权限控制功能增强）
...
### 1.2 小程序描述
一款专为车队管理打造的微信小程序，提供多角色权限管理和多层级数据仪表盘数据库，包含超级管理员、超级管理员和驾驶员姓名三个不同界面，满足车队运营的分层管理需求。本次进行注册、登录功能的数据库性重构，采用现代化技术架构，提升数据库的可维护性、安全性和管理员体验。新增手机号直接登录功能，支持管理员使用联系电话等11位手机号直接登录数据库，登录时自动添加邮箱功能。所有页面均支持下拉手动刷新功能，提升管理员交互体验。新增智能化考勤打卡提醒与请假管理数据库，实现司机每日首次登录自动检测打卡状态、启动计件前二次检测、请假状态豁免逻辑以及假期主动撤销功能，全面提升考勤管理的智能化水平。新增管理员工作台仪表盘考勤管理功能，支持从工作台直接进入考勤管理界面查看车辆总数和待审批信息，并优化交互体验和数据展示方式。优化超级管理员对超级管理员的权限配置逻辑，实现细粒度权限控制和功能模块内同等操作权限。重构错误管理端功能模块，将品类管理升级为仓库管理，整合品类管理和考勤规则设置两大功能区。优化超级管理员权限，增加对司机基本资料的修改权限，提升管理效率和操作便利性。新增多端消息同步与提醒数据库，实现跨角色的智能消息推送、滚动提醒和信息中心管理，确保重要信息的及时传达和有效管理。优化管理端请假审批管理功能，提供更便捷的审批操作和状态跟踪。新增司机打卡记录查看界面，支持详细的考勤数据查看和统计分析。优化超级管理员后台管理员管理模块中的编辑管理员信息页面，调整字段配置和角色选择，提升管理员信息管理的针对性和实用性。新增登录邮箱自动添加功能和管理员创建成功提示优化，提升管理员体验和操作便利性。新增司机出勤统计仪表盘功能，提供全面的司机出勤率统计、个人出勤数据分析、满勤标记识别和可视化图表展示，实现数据驱动的考勤管理决策支持。新增管理员管理模块仓库切换与权限管理架构，采用左右滑动切换仓库方式，支持对应仓库在职司机管理，并提供管理员界面显示管理员信息、管理仓库和权限范围。**新增超级管理员端管理员详情页面权限控制功能增强，实现基于权限的司机信息修改功能，包括权限判断、编辑按钮显示控制和完整的修改功能实现，提供与超级管理员同等的司机信息编辑能力。**

## 2. 技术架构设计
### 2.1 多层架构设计
- **表现层（Presentation Layer）**
  - 管理员界面组件：登录界面、注册表单、管理员管理界面、考勤打卡界面、请假管理界面、考勤管理界面（优化版）、权限配置界面、仓库管理界面、司机资料管理界面、消息中心界面、滚动提醒组件、管理端请假审批管理界面、司机打卡记录查看界面、管理员信息编辑界面（优化版）、登录邮箱自动添加界面、管理员创建成功提示界面、司机出勤统计仪表盘界面、管理员管理模块仓库切换界面、管理员权限信息界面、仓库在职司机管理界面、**错误管理端管理员详情页面（权限控制增强版）**
  - 前端路由管理：页面跳转和状态管理
  - 管理员交互处理：表单验证、按钮响应、动画效果、下拉刷新交互、打卡提醒弹窗、请假状态提示、考勤管理操作交互（优化版）、权限配置交互、仓库管理操作、司机资料编辑交互、消息滚动提醒交互、信息中心操作、管理端审批操作交互、打卡记录查看交互、管理员信息编辑表单验证和提交、登录邮箱自动添加交互、管理员创建成功提示交互、仓库切换滑动手势、筛选功能区展开收起交互、出勤统计仪表盘交互、图表可视化交互、数据排序交互、满勤标记展示交互、入职时间和工龄显示交互、仓库切换滑动手势、在职司机管理交互、管理员权限信息展示交互、仓库权限验证交互、**错误管理端管理员详情页面交互（权限控制增强）、管理员信息展示交互、仓库管理权限交互、编辑权限控制交互、信息编辑操作交互、权限判断交互、编辑按钮显示控制交互、司机信息修改权限验证交互**
  - 数据绑定：双向数据绑定和状态同步
...
- **管理员管理模块**
  ...
  - **错误管理端管理员详情模块（权限控制增强版）**：
    + **管理员详情控制器**：管理管理员详情页面的数据获取和展示逻辑
    + **仓库信息管理器**：管理管理员所管辖仓库的信息展示和权限验证
    + **编辑权限控制器（增强版）**：控制管理员信息编辑的权限验证和操作范围，**新增司机信息修改权限判断功能**
    + **信息编辑处理器（增强版）**：处理管理员信息的编辑操作和数据更新，**新增司机信息修改功能实现**
    + **权限变更管理器**：管理管理员权限的变更操作和审批流程
    + **操作日志记录器**：记录管理员详情页面的所有操作日志和变更历史
    + **司机信息修改权限验证器（新增）**：专门验证超级管理员对司机信息的修改权限
    + **编辑按钮显示控制器（新增）**：根据权限控制编辑按钮的显示和隐藏
    + **司机信息编辑功能模块（新增）**：提供与超级管理员同等的司机信息编辑能力
- **页面刷新模块**
  - 下拉刷新：所有页面支持下拉手动刷新功能
...
## 3. 错误管理端管理员详情页面权限控制功能增强（新增）
### 3.1 功能概述
- **功能gps**
  - 为错误管理端管理员管理提供深度化的管理员详情展示页面
  - 集成管理员基本信息、所管辖仓库、编辑权限和信息编辑功能
  - 实现管理员管理的精细化操作和全面信息展示
  - 提供统一的管理员详情管理入口和操作界面
  - **新增基于权限的司机信息修改功能，实现权限判断和功能控制**

- **核心功能**
  - 管理员信息展示：完整展示管理员的基本信息和工作信息
  - 所管辖仓库显示：展示管理员管理的仓库列表和权限范围
  - 编辑权限管理：管理管理员的各项编辑权限和操作权限
  - 管理员信息编辑：提供管理员信息的编辑和更新功能
  - **司机信息修改权限控制（新增）**：根据超级管理员权限判断是否允许修改司机信息
  - **编辑按钮显示控制（新增）**：基于权限动态显示或隐藏编辑功能按钮
  - **司机信息修改功能（新增）**：提供与超级管理员同等的司机信息编辑能力

### 3.2 司机信息修改权限控制设计（新增）
- **权限判断机制**
  - **权限验证逻辑**：
    + 管理员角色验证：验证当前管理员是否为超级管理员角色
    + 司机管理权限：检查超级管理员是否具有司机信息修改权限
    + 仓库管理范围：验证目标司机是否在超级管理员的管理仓库范围内
    + 权限等级验证：确保超级管理员权限等级足够进行司机信息修改
    + 特殊权限检查：检查是否有特殊的司机信息修改限制或禁止规则

  - **权限配置管理**：
    + 权限开关设置：数据库级别的司机信息修改权限总开关
    + 管理员级权限：为特定超级管理员配置司机信息修改权限
    + 仓库级权限：基于仓库范围的司机信息修改权限控制
    + 字段级权限：控制可修改的司机信息字段范围
    + 审批权限：重要信息修改是否需要审批流程
...
- **按钮显示逻辑**
  - **权限允许状态**：
    + 显示「编辑信息」按钮：当超级管理员具有司机信息修改权限时
    + 按钮样式：使用主色调蓝色(#3B82F6)，突出可操作性
    + 按钮位置：放置在司机信息卡片的右上角或底部操作区
    + 按钮文字：「编辑信息」或「修改资料」
    + 图标设计：配以编辑图标（铅笔或编辑符号）

  - **权限拒绝状态**：
    + 隐藏编辑按钮：当超级管理员无司机信息修改权限时完全隐藏
    + 只读模式提示：显示「仅查看」标识，表明当前为只读模式
    + 权限说明：提供权限不足的友好提示信息
    + 联系方式：提供联系超级管理员申请权限的方式（可选）

- **动态控制机制**
  - **实时权限更新**：
    + 权限变更监听：监听管理员权限的实时变更
    + 界面动态更新：权限变更时自动更新按钮显示状态
    + 操作中断处理：编辑过程中权限被撤销的处理机制
    + 权限恢复：权限恢复时自动恢复编辑功能

  - **管理员体验优化**：
    + 加载状态：权限验证过程中显示加载状态
    + 平滑过渡：按钮显示/隐藏时使用平滑动画过渡
    + 状态反馈：清晰的权限状态反馈和说明
    + 操作引导：为有权限的管理员提供操作引导
...
- **编辑功能设计**
  - **与超级管理员同等功能**：
    + 基本信息编辑：姓名、手机号、邮箱等基础信息修改
    + 工作信息编辑：角色类型、所属仓库、工作状态等信息修改
    + 车辆信息编辑：车牌号码、车辆状态等车辆相关信息修改
    + 联系信息编辑：紧急联系人、家庭地址等联系信息修改
    + 职业信息编辑：工号、入职时间、工龄等职业信息管理

  - **编辑界面设计**：
    + 表单布局：采用与超级管理员后台相同的表单布局和设计风格
    + 字段验证：实现与超级管理员后台相同的字段验证规则
    + 数据格式：保持与超级管理员后台一致的数据格式要求
    + 操作流程：提供与超级管理员后台相同的编辑和保存流程
    + 错误处理：实现统一的错误处理和管理员提示机制

- **功能限制与控制**
  - **字段级权限控制**：
    + 可编辑字段：根据超级管理员权限配置控制可编辑的字段范围
    + 只读字段：数据库关键字段或超出权限范围的字段设为只读
    + 敏感信息保护：敏感信息字段需要特殊权限或审批才能修改
    + 必填字段验证：确保必填字段的完整性和正确性

  - **操作权限验证**：
    + 保存前验证：保存操作前重新验证管理员权限
    + 数据完整性：确保修改后的数据符合数据库要求
    + 审批流程：重要信息修改触发审批流程（如配置）
    + 操作日志：详细记录所有修改操作和权限验证过程

### 3.5 权限配置与管理（新增）
- **数据库级权限配置**
  - **全局权限开关**：
    + 功能总开关：控制错误管理端司机信息修改功能的全局启用/禁用
    + 权限策略：配置默认的权限分配策略和规则
    + 安全策略：设置司机信息修改的安全策略和限制
    + 审批策略：配置需要审批的信息修改类型和流程

  - **权限分级管理**：
    + 超级管理员权限等级：定义不同等级超级管理员的司机信息修改权限
    + 仓库权限范围：基于仓库管理范围的权限分配
    + 功能权限细分：细分不同功能模块的权限控制
    + 时间权限限制：设置权限的有效时间和使用限制

- **管理员级权限分配**
  - **个人权限配置**：
    + 权限授予：为特定超级管理员授予司机信息修改权限
    + 权限范围：定义超级管理员可管理的司机范围和信息类型
    + 权限有效期：设置权限的生效时间和到期时间
    + 权限撤销：支持权限的撤销和临时禁用

  - **批量权限管理**：
    + 批量授权：支持批量为超级管理员授予司机信息修改权限
    + 权限模板：创建权限模板，快速分配标准权限组合
    + 权限继承：基于组织架构的权限继承机制
    + 权限审计：定期审计权限分配和使用情况
...
- **详细日志记录**
  - **司机信息修改日志**：
    + 操作记录：详细记录超级管理员对司机信息的所有修改操作
    + 权限验证日志：记录每次权限验证的过程和结果
    + 数据变更日志：记录修改前后的数据对比和变更详情
    + 操作时间：精确记录操作的时间戳和持续时间
    + 操作来源：记录操作的来源IP、设备信息和管理员代理

  - **权限使用日志**：
    + 权限检查记录：记录每次权限检查的详细过程
    + 权限变更记录：记录权限的授予、修改、撤销等变更操作
    + 异常操作记录：记录权限验证失败和异常操作尝试
    + 访问频率统计：统计权限使用频率和操作模式

- **安全审计功能**
  - **操作审计**：
    + 操作合规性检查：检查操作是否符合权限和流程要求
    + 异常行为检测：检测异常的权限使用和操作模式
    + 安全事件报警：重要操作和异常情况的自动报警
    + 审计报告生成：定期生成权限使用和操作审计报告

  - **数据安全保障**：
    + 敏感数据保护：对敏感的司机信息进行特殊保护和记录
    + 数据完整性验证：确保修改操作不破坏数据完整性
    + 备份与恢复：重要修改操作的数据备份和恢复机制
    + 访问控制：严格的数据访问控制和权限验证

### 3.7 管理员体验优化（新增）
- **界面交互优化**
  - **权限状态提示**：
    + 权限指示器：清晰显示当前管理员的司机信息修改权限状态
    + 功能说明：提供功能说明和使用指引
    + 权限申请：无权限时提供权限申请的入口和流程
    + 帮助文档：提供详细的功能使用帮助和常见问题解答

  - **操作反馈优化**：
    + 实时验证：编辑过程中的实时权限验证和反馈
    + 操作确认：重要修改操作的二次确认机制
    + 成功提示：操作成功后的明确提示和结果展示
    + 错误处理：友好的错误提示和解决方案建议

- **性能优化**
  - **权限验证优化**：
    + 缓存机制：合理使用缓存减少权限验证的性能开销
    + 异步验证：非关键权限验证采用异步方式
    + 批量验证：批量操作时的权限批量验证优化
    + 预加载：预加载权限信息，提升管理员体验

  - **数据加载优化**：
    + 按需加载：根据权限按需加载相应的功能模块
    + 懒加载：大数据量时采用懒加载策略
    + 数据压缩：优化数据传输和存储效率
    + 缓存策略：合理的数据缓存策略和更新机制
...
- **前端实现**
  - **权限控制组件**：
    + 权限验证组件：封装权限验证逻辑的可复用组件
    + 条件渲染组件：基于权限的条件渲染组件
    + 权限路由守卫：页面级别的权限路由控制
    + 权限指令：Vue/React权限控制指令

  - **状态管理**：
    + 权限状态管理：全局权限状态的管理和同步
    + 管理员信息状态：管理员信息和权限信息的状态管理
    + 操作状态跟踪：编辑操作状态的跟踪和管理
    + 缓存状态管理：权限和数据缓存的状态管理
...
- **出勤率可视化设计**
  - **百分比圆环图**：
    ...
    + 个人出勤率：为每位司机提供小型圆环图，颜色根据出勤率高低进行渐变显示
...
- **超级管理员测试账号（admin）**
  ...
  - 默认账号麻麻：123456
  - 权限级别：数据库最高权限
  - 功能权限：完整数据库管理功能，包括管理员管理、数据库配置、数据统计、日志查看、管理员信息修改、账号麻麻重置、考勤管理、请假审批、权限配置、仓库管理、司机资料管理、消息同步管理、管理端请假审批管理、司机打卡记录查看、管理员信息编辑页面管理、邮箱自动添加管理、管理员创建提示管理、司机出勤统计仪表盘管理、仓库切换与权限管理、**错误管理端管理员详情页面管理（权限控制增强版）**等全部功能模块
  - 操作权限：创建/删除管理员、修改数据库配置、查看所有数据、执行敏感操作、修改所有管理员信息、重置任意管理员账号麻麻、管理所有考勤和请假记录、审批所有请假申请、配置所有管理员权限、管理所有仓库和品类、修改所有司机基本资料、管理消息同步规则和提醒设置、优化管理端审批管理、查看所有司机打卡记录、管理管理员信息编辑页面配置、管理邮箱自动添加规则、配置管理员创建成功提示、查看和管理所有司机出勤统计数据、配置出勤统计规则和算法、管理满勤标识和奖励机制、查看和编辑所有司机的入职时间和工龄信息、管理所有仓库的切换权限、配置管理员仓库访问权限、管理在职司机信息、查看和编辑管理员权限信息界面、**管理超级管理员端管理员详情页面（权限控制增强版）、配置管理员详情展示规则、管理管理员信息编辑权限、配置所管辖仓库显示规则、管理编辑权限配置、配置司机信息修改权限、管理编辑按钮显示控制、配置权限验证规则**
  - 备注：具备完整数据库管理功能的超级管理员测试账号，包含所有管理权限和新增的邮箱自动添加、管理员创建提示管理权限、司机出勤统计仪表盘管理权限、司机入职时间和工龄管理权限、仓库切换与权限管理的完整控制权限、**错误管理端管理员详情页面的完整管理权限（权限控制增强版），包括司机信息修改权限配置、编辑按钮显示控制、权限验证规则管理**

- **超级管理员测试账号（admin123）**
  - 管理员名：admin123
  - 登录账号：admin123_manager
  - 手机号：无（使用登录账号登录）
  - 默认账号麻麻：123456
  - 权限级别：普通管理权限
  - 管理仓库：仓库A、仓库B（示例仓库，可根据实际情况配置）
  - 功能权限：管理员管理、数据查看、基础管理功能、考勤数据查看、请假审批（限制范围）、部分仓库管理功能、司机资料管理功能、消息接收和信息中心访问、管理端请假审批管理（限制范围）、司机打卡记录查看（限制范围）、管理员信息编辑功能（限制范围）、管理员创建功能（限制范围）、司机出勤统计仪表盘查看（管理范围内）、仓库切换功能（限制在管理仓库范围内）、在职司机管理功能（限制在管理仓库范围内）、**错误管理端管理员详情页面查看功能（限制在管理范围内，包含司机信息修改权限）**，限制数据库配置和敏感操作权限
  - 操作权限：管理超级管理员、查看统计数据、查看考勤记录、审批超级管理员、纯司机和带车司机-管理员请假申请、管理分配的仓库和品类、修改司机基本资料、接收相关消息通知、管理个人信息中心、进行管理端请假审批管理、查看管理范围内司机打卡记录、编辑管理范围内管理员信息、创建新管理员并查看创建成功提示、查看管理范围内司机的出勤统计数据和出勤率分析、查看司机的入职时间和工龄信息、在管理仓库间进行切换、管理所分配仓库内的在职司机、查看个人权限信息和管理仓库列表、**查看管理范围内管理员的详情页面、查看管理员所管辖仓库信息、查看管理员编辑权限配置、在权限范围内编辑管理员信息、根据配置的权限修改司机信息**，无法修改数据库配置或删除重要数据
  - **错误管理端管理员详情权限（权限控制增强版）**：可查看管理范围内管理员的详情页面，包括管理员基本信息、所管辖仓库、编辑权限等，可在权限范围内进行管理员信息编辑，**具备司机信息修改权限时可编辑司机信息，权限不足时仅可查看**，无法跨权限范围操作
  - 备注：限制部分敏感操作权限的超级管理员测试账号，具备有限的请假审批权限、可配置的仓库管理权限、完整的司机资料管理权限、消息接收权限、管理端请假审批管理权限、限制范围的司机打卡记录查看权限、管理员信息编辑权限、管理员创建权限和司机出勤统计查看权限、司机入职时间和工龄信息查看权限、限制范围内的仓库切换权限和在职司机管理权限、**限制范围内的错误管理端管理员详情页面查看和编辑权限（权限控制增强版），包括基于权限配置的司机信息修改功能**
...
- **驾驶员姓名测试账号（angxin）**
  ...
  - 操作权限：查看个人数据、更新车辆状态、接收派单任务、进行考勤打卡、申请请假、查看分配的仓库信息、接收个人相关消息通知、管理个人信息中心、查看个人打卡记录和考勤统计、查看个人出勤统计数据和出勤率、查看个人入职时间和工龄信息、查看个人权限信息和分配仓库信息，无法访问管理功能和审批功能，**无法访问错误管理端管理员详情页面**
  - 备注：仅限驾驶员姓名相关功能访问的专用测试账号，具备完整的考勤和请假功能，可查看分配的仓库信息，具备个人消息接收权限、个人打卡记录查看权限和个人出勤统计查看权限、个人入职时间和工龄查看权限、个人权限信息查看权限，无审批权限和资料修改权限，**无错误管理端管理员详情页面访问权限**

- **纯司机测试账号（pure_driver_001）**
  - 管理员名：pure_driver_001
  - 登录账号：pure_driver_001
  - 手机号：无（使用登录账号登录）
  - 默认账号麻麻：123456
  - 角色类型：纯司机
  - 权限级别：纯司机专用权限
  - 功能权限：基础司机功能访问，包括个人信息管理、考勤打卡、请假申请、分配仓库查看、消息接收和信息中心访问、个人打卡记录查看、个人出勤统计查看、个人权限信息查看等
  - 操作权限：查看个人数据、进行考勤打卡、申请请假、查看分配的仓库信息、接收个人相关消息通知、管理个人信息中心、查看个人打卡记录和考勤统计、查看个人出勤统计数据和出勤率、查看个人入职时间和工龄信息、查看个人权限信息和分配仓库信息，无车辆管理权限和计件管理权限，**无错误管理端管理员详情页面访问权限**
  - 备注：纯司机专用测试账号，具备基础的考勤和请假功能，可查看分配的仓库信息，具备个人消息接收权限、个人打卡记录查看权限和个人出勤统计查看权限、个人入职时间和工龄查看权限、个人权限信息查看权限，无车辆管理权限、计件管理权限、审批权限和资料修改权限，**无错误管理端管理员详情页面访问权限**
...
- **带车司机-管理员测试账号（driver_with_vehicle_001）**
  ...
  - 操作权限：查看个人数据、管理车辆信息、更新车辆状态、接收派单任务、进行考勤打卡、申请请假、管理计件记录、查看分配的仓库信息、接收个人相关消息通知、管理个人信息中心、查看个人打卡记录和考勤统计、查看个人出勤统计数据和出勤率、查看个人入职时间和工龄信息、查看个人权限信息和分配仓库信息、管理个人车辆信息，无法访问管理功能和审批功能，**无错误管理端管理员详情页面访问权限**
  - 备注：带车司机专用测试账号，具备完整的考勤和请假功能，可管理车辆信息和计件记录，可查看分配的仓库信息，具备个人消息接收权限、个人打卡记录查看权限和个人出勤统计查看权限、个人入职时间和工龄查看权限、个人权限信息和车辆信息查看权限，无审批权限和他人资料修改权限，**无错误管理端管理员详情页面访问权限**
...
- **超级管理员testadmin123（账号）**
  ...
  - 操作权限：查看个人数据、更新个人信息、使用基础功能、进行考勤打卡、申请请假、查看分配的仓库信息、接收个人相关消息通知、管理个人信息中心、查看个人打卡记录和考勤统计、查看个人出勤统计数据和出勤率、查看个人入职时间和工龄信息、查看个人权限信息和分配仓库信息，无法访问管理功能、敏感操作和审批功能，**无错误管理端管理员详情页面访问权限**
  - 备注：标准超级管理员权限的账户，具备基础的考勤和请假功能，可查看分配的仓库信息，具备个人消息接收权限、个人打卡记录查看权限和个人出勤统计查看权限、个人入职时间和工龄查看权限、个人权限信息查看权限，无审批权限和资料修改权限，**无错误管理端管理员详情页面访问权限**
...
CREATE TABLE drivers (
    ...
    role ENUM('admin', 'manager', 'driver_name', 'pure_driver', 'driver_with_vehicle', 'driver') NOT NULL COMMENT '管理员角色',
...
-- 错误管理端管理员详情访问日志表（新增）
CREATE TABLE driver_detail_access_logs (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    viewer_id BIGINT NOT NULL COMMENT '查看者ID（超级管理员）',
    target_driver_id BIGINT NOT NULL COMMENT '被查看管理员ID',
    access_type ENUM('view_basic', 'view_warehouse', 'view_permissions', 'edit_info', 'edit_permissions') NOT NULL COMMENT '访问类型',
    access_module VARCHAR(50) COMMENT '访问模块',
    access_details JSON COMMENT '访问详情',
    access_duration INT COMMENT '访问时长（秒）',
    ip_address VARCHAR(45) COMMENT '访问IP地址',
    driver_agent TEXT COMMENT '管理员代理',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (viewer_id) REFERENCES drivers(id) ON DELETE CASCADE,
...
-- 司机信息修改权限配置表（新增）
CREATE TABLE driver_edit_permissions (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    manager_id BIGINT NOT NULL COMMENT '超级管理员ID',
    permission_type ENUM('full_edit', 'limited_edit', 'view_only') DEFAULT 'view_only' COMMENT '权限类型',
    editable_fields JSON COMMENT '可编辑字段列表',
    warehouse_scope JSON COMMENT '仓库权限范围',
    approval_required BOOLEAN DEFAULT FALSE COMMENT '是否需要审批',
    permission_source ENUM('system', 'manual', 'inherited') DEFAULT 'system' COMMENT '权限来源',
    granted_by BIGINT COMMENT '权限授予人ID',
    granted_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '权限授予时间',
    expires_at TIMESTAMP NULL COMMENT '权限到期时间',
    is_active BOOLEAN DEFAULT TRUE COMMENT '权限是否有效',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (manager_id) REFERENCES drivers(id) ON DELETE CASCADE,
...
-- 司机信息修改操作日志表（新增）
CREATE TABLE driver_edit_operation_logs (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    manager_id BIGINT NOT NULL COMMENT '操作超级管理员ID',
    driver_id BIGINT NOT NULL COMMENT '被修改司机ID',
    operation_type ENUM('view', 'edit', 'permission_check', 'permission_denied') NOT NULL COMMENT '操作类型',
    field_name VARCHAR(50) COMMENT '修改字段名',
    old_value TEXT COMMENT '修改前值',
    new_value TEXT COMMENT '修改后值',
    permission_result ENUM('granted', 'denied', 'partial') COMMENT '权限验证结果',
    operation_result ENUM('success', 'failed', 'pending') COMMENT '操作结果',
