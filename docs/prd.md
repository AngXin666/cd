...
### 1.3 项目目标
快速开发一个可登录的车队管理小程序，实现基础的车辆、司机管理和数据统计功能，基于独立数据库模式确保数据完全隔离，并优化数据库结构和查询逻辑，完善账号权限管理和通知系统。
...
### 5.1 核心目标
优化数据库结构，明确各账号角色的查询权限，确保所有查询均基于老板的独立数据库，简化查询逻辑，提升数据访问效率。
...
- **车队长账号**：限定查询管辖范围内的数据
- **司机账号**：仅能查询个人相关信息
- **统一数据库上下文**：所有查询自动关联当前老板的独立数据库

### 5.4 查询逻辑优化
- **默认数据库绑定**：系统自动识别并绑定当前老板的独立数据库
- **角色权限过滤**：根据用户角色自动添加查询条件
- **简化查询语法**：开发者无需手动拼接复杂的权限条件
- **统一数据源**：所有账号查询基于相同的数据库结构
...
- 图标风格：线性图标风格，简洁现代
- 按钮样式：主操作按钮采用蓝色渐变，禁用状态呈灰色
- 通知图标：采用铃铛样式，新通知时显示红点提示

### 7.3 整体布局
- 采用卡片式布局，内容分区清晰
- 左右分栏结构，便于数据对比查看
- 顶部导航栏固定，底部标签栏便于快速切换
- 登录页面采用居中对齐设计，突出登录框和提示信息
- 通知区域：在顶部导航栏显示通知图标和未读数量
...
- **权限范围**：仅能查看和管理自己的个人信息
- **操作限制**：严禁查看其他司机的任何数据
- **可操作事项**：发起请假申请、离职申请、车辆审核申请、实名审核申请

### 9.2 关键操作通知要求
- **所有关键操作均需生成系统通知**
- **触发通知的操作**：
  - 司机提交：请假申请、离职申请、车辆信息审核申请、实名认证审核申请
  - 车队长执行：增加司机、修改司机信息（分配仓库、修改类型）、停用司机账号、删除司机账号（如果被授权）
  - 任何账号进行的审核操作（通过/驳回）

### 9.3 通知发送与接收规则
- **通知接收方**：
  - **必送对象**：所有通知必须发送给老板账号及其管辖下的所有平级账号
  - **属地对象**：通知必须发送给操作发起人所属管辖区的车队长账号