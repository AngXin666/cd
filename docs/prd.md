# 车队管理小程序需求文档（技术重构版 - 手机号登录功能增强 + 智能考勤打卡提醒与请假管理 + 考勤管理界面 + 权限配置优化与仓库管理模块重构 + 超级管理员司机资料管理权限优化 + 多端消息同步与提醒数据库 + 管理端请假审批管理优化 + 司机打卡记录查看界面 + 管理员管理模块编辑页面优化 + 登录邮箱自动添加与管理员创建优化）
...
- **数据访问层（Data Access Layer）**
  ...
  - 管理员创建数据访问：管理员创建记录、默认密码设置、创建成功日志
...
- **管理员管理模块**
  - 管理员注册：多渠道注册支持、信息验证、自动激活、默认密码自动设置
  ...
  - **管理员信息编辑模块（优化版）**：
    ...
    + 数据更新：管理员信息实时更新和同步机制
  - **管理员创建成功提示模块（新增）**：
    + 信息展示：姓名、手机号、司机类型、登录账号、密码、车牌号码完整信息展示
...
- **自动识别机制**
  ...
  - 登录方式判断：区分手机号+密码登录和手机号+验证码登录两种方式
...
### 4.5 自动密码设置机制
- **默认密码规则**
  - 统一密码：所有新创建管理员的默认密码统一设置为「123456」
  - 密码强度：虽为简单密码，但要求管理员首次登录后强制修改
  - 安全提示：在提示信息中明确提醒管理员及时修改默认密码

- **密码设置流程**
  1. 管理员创建时自动设置默认密码123456
  2. 密码经过加密处理后存储到数据库
  3. 标记该密码为默认密码状态
  4. 在创建成功提示中明文显示密码供管理员记录
  5. 管理员首次登录时提示修改密码
...
### 6.2 邮箱自动添加逻辑实现
- **触发条件检测**
  - 登录方式识别：检测是否为手机号登录（手机号+密码或手机号+验证码）
...
### 7.2 默认密码自动设置机制（新增）
- **统一默认密码规则**
  - 默认密码：所有新创建管理员自动设置默认密码为「123456」
  - 密码加密：使用bcrypt对默认密码进行加密存储
  - 状态标记：在管理员密码表中标记为默认密码状态
  - 强制修改：管理员首次登录时强制要求修改默认密码

- **密码设置流程**
  1. 管理员注册信息验证通过
  2. 数据库自动为新管理员设置默认密码123456
  3. 密码经过bcrypt加密处理
...
- **账号麻麻复杂度要求**
  ...
  - **默认密码处理**：默认密码123456不受复杂度要求限制，但要求首次登录后修改
- **账号麻麻安全存储**
  - 加密算法：使用bcrypt进行账号麻麻哈希
  - 盐值生成：随机盐值生成和存储
  - 存储分离：账号麻麻哈希与管理员信息分表存储
  - 默认密码标记：特别标记默认密码状态，便于管理
...
- **分步骤注册**
  ...
  - 第四步：补充信息（车牌号码等可选信息）
  - 第五步：信息确认（最终确认提交）
  - **第六步：创建成功提示（新增）**：显示包含默认密码的完整管理员信息
- **实时验证机制**
...
- **超级管理员测试账号（admin）**
  ...
  - 默认密码：123456
  - 权限级别：数据库最高权限
  - 功能权限：完整数据库管理功能，包括管理员管理、数据库配置、数据统计、日志查看、管理员信息修改、账号麻麻重置、考勤管理、请假审批、权限配置、仓库管理、司机资料管理、消息同步管理、管理端请假审批管理、司机打卡记录查看、管理员信息编辑页面管理、邮箱自动添加管理、管理员创建提示管理等全部功能模块
  - 操作权限：创建/删除管理员、修改数据库配置、查看所有数据、执行敏感操作、修改所有管理员信息、重置任意管理员账号麻麻、管理所有考勤和请假记录、审批所有请假申请、配置所有管理员权限、管理所有仓库和品类、修改所有司机基本资料、管理消息同步规则和提醒设置、优化管理端审批管理、查看所有司机打卡记录、管理管理员信息编辑页面配置、管理邮箱自动添加规则、配置管理员创建成功提示
  - 备注：具备完整数据库管理功能的超级管理员测试账号，包含所有管理权限和新增的邮箱自动添加、管理员创建提示管理权限
...
- **超级管理员测试账号（admin123）**
  ...
  - 默认密码：123456
  - 权限级别：普通管理权限
  - 功能权限：管理员管理、数据查看、基础管理功能、考勤数据查看、请假审批（限制范围）、部分仓库管理功能、司机资料管理功能、消息接收和信息中心访问、管理端请假审批管理（限制范围）、司机打卡记录查看（限制范围）、管理员信息编辑功能（限制范围）、管理员创建功能（限制范围），限制数据库配置和敏感操作权限
  - 操作权限：管理超级管理员、查看统计数据、查看考勤记录、审批超级管理员、纯司机和带车司机-管理员请假申请、管理分配的仓库和品类、修改司机基本资料、接收相关消息通知、管理个人信息中心、进行管理端请假审批管理、查看管理范围内司机打卡记录、编辑管理范围内管理员信息、创建新管理员并查看创建成功提示，无法修改数据库配置或删除重要数据
  ...
  - 管理员创建权限：可创建新管理员，创建成功后可查看包含默认密码的完整提示信息
  - 备注：限制部分敏感操作权限的超级管理员测试账号，具备有限的请假审批权限、可配置的仓库管理权限、完整的司机资料管理权限、消息接收权限、管理端请假审批管理权限、限制范围的司机打卡记录查看权限、管理员信息编辑权限和管理员创建权限
...
- **登录可用性验证**
  ...
  - **默认密码测试**：验证新管理员默认密码123456的设置和首次登录强制修改功能
...
('admin', 'driver5_admin', '联系电话', '联系电话@company.com', 'auto', 'admin', 'active', '超级管理员', NULL),
('admin123', 'admin123_manager', NULL, NULL, 'auto', 'manager', 'active', '超级管理员', NULL),
('driver10', 'angxin_driver_name', NULL, NULL, 'auto', 'driver_name', 'active', '驾驶员姓名管理员', NULL),
