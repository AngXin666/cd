# 考勤管理功能使用指南

## 📋 概述

本文档介绍了车队管家小程序中考勤管理功能的使用方法，包括权限设计、API 使用和前端集成。

---

## 🔐 权限设计

### 角色权限矩阵

| 操作 | BOSS | MANAGER | DRIVER |
|-----|------|---------|--------|
| 查看所有考勤记录 | ✅ | ✅ | ❌ |
| 查看自己的考勤记录 | ✅ | ✅ | ✅ |
| 创建考勤记录 | ✅ | ✅ | ✅ |
| 更新所有考勤记录 | ✅ | ✅ | ❌ |
| 更新自己未完成的考勤 | ✅ | ✅ | ✅ |
| 删除考勤记录 | ✅ | ✅ | ❌ |

### RLS 策略说明

#### attendance 表策略

1. **new_admins_view_all_attendance** (SELECT)
   - 管理员可以查看所有考勤记录
   - 规则: `is_admin(auth.uid())`

2. **new_drivers_view_own_attendance** (SELECT)
   - 司机可以查看自己的考勤记录
   - 规则: `user_id = auth.uid()`

3. **new_admins_insert_attendance** (INSERT)
   - 管理员可以创建考勤记录
   - 规则: `is_admin(auth.uid())`

4. **new_drivers_insert_own_attendance** (INSERT)
   - 司机可以创建自己的考勤记录
   - 规则: `user_id = auth.uid()`

5. **new_admins_update_all_attendance** (UPDATE)
   - 管理员可以更新所有考勤记录
   - 规则: `is_admin(auth.uid())`

6. **new_drivers_update_own_attendance** (UPDATE)
   - 司机可以更新自己未完成的考勤记录
   - 规则: `user_id = auth.uid() AND clock_out_time IS NULL`

7. **new_admins_delete_attendance** (DELETE)
   - 管理员可以删除考勤记录
   - 规则: `is_admin(auth.uid())`

---

## 🛠️ 数据库函数

### 1. 打卡上班

```sql
clock_in(p_user_id uuid, p_warehouse_id uuid DEFAULT NULL, p_notes text DEFAULT NULL) 
RETURNS uuid
```

**功能**: 打卡上班，创建考勤记录

**参数**:
- `p_user_id`: 用户 ID
- `p_warehouse_id`: 仓库 ID（可选）
- `p_notes`: 备注（可选）

**返回**: 考勤记录 ID

**异常**:
- 今天已经打过卡了

**示例**:
```sql
SELECT clock_in(
  '用户ID',
  '仓库ID',
  '正常上班'
);
```

---

### 2. 打卡下班

```sql
clock_out(p_user_id uuid, p_notes text DEFAULT NULL) 
RETURNS boolean
```

**功能**: 打卡下班，更新考勤记录

**参数**:
- `p_user_id`: 用户 ID
- `p_notes`: 备注（可选）

**返回**: 布尔值，true 表示成功

**异常**:
- 今天还没有打卡上班，或者已经打卡下班了

**示例**:
```sql
SELECT clock_out(
  '用户ID',
  '正常下班'
);
```

---

### 3. 获取用户的考勤记录

```sql
get_user_attendance(p_user_id uuid, p_start_date date DEFAULT NULL, p_end_date date DEFAULT NULL)
RETURNS TABLE (...)
```

**功能**: 获取用户的考勤记录

**参数**:
- `p_user_id`: 用户 ID
- `p_start_date`: 开始日期（可选）
- `p_end_date`: 结束日期（可选）

**返回**: 考勤记录列表

**示例**:
```sql
-- 获取用户本月的考勤记录
SELECT * FROM get_user_attendance(
  '用户ID',
  '2025-12-01',
  '2025-12-31'
);
```

---

### 4. 获取用户今天的考勤状态

```sql
get_today_attendance_status(p_user_id uuid)
RETURNS TABLE (has_clocked_in boolean, has_clocked_out boolean, ...)
```

**功能**: 获取用户今天的考勤状态

**参数**:
- `p_user_id`: 用户 ID

**返回**: 今天的考勤状态

**示例**:
```sql
SELECT * FROM get_today_attendance_status('用户ID');
```

---

### 5. 获取考勤统计

```sql
get_attendance_statistics(p_user_id uuid, p_start_date date, p_end_date date)
RETURNS TABLE (total_days integer, present_days integer, ...)
```

**功能**: 获取考勤统计

**参数**:
- `p_user_id`: 用户 ID
- `p_start_date`: 开始日期
- `p_end_date`: 结束日期

**返回**: 考勤统计数据

**示例**:
```sql
-- 获取用户本月的考勤统计
SELECT * FROM get_attendance_statistics(
  '用户ID',
  '2025-12-01',
  '2025-12-31'
);
```

---

### 6. 管理员获取所有用户的考勤记录

```sql
get_all_attendance(p_admin_id uuid, p_start_date date DEFAULT NULL, p_end_date date DEFAULT NULL)
RETURNS TABLE (...)
```

**功能**: 管理员获取所有用户的考勤记录

**参数**:
- `p_admin_id`: 管理员 ID
- `p_start_date`: 开始日期（可选）
- `p_end_date`: 结束日期（可选）

**返回**: 所有用户的考勤记录

**异常**:
- 只有管理员可以查看所有考勤记录

**示例**:
```sql
-- 获取今天所有用户的考勤记录
SELECT * FROM get_all_attendance(
  '管理员ID',
  CURRENT_DATE,
  CURRENT_DATE
);
```

---

## 💻 前端集成

### 1. 打卡上班

```typescript
import { supabase } from '@/client/supabase';
import Taro from '@tarojs/taro';

async function clockIn(warehouseId?: string, notes?: string) {
  const { data: { user } } = await supabase.auth.getUser();
  
  if (!user) {
    throw new Error('用户未登录');
  }
  
  const { data, error } = await supabase
    .rpc('clock_in', {
      p_user_id: user.id,
      p_warehouse_id: warehouseId || null,
      p_notes: notes || null
    });
  
  if (error) {
    throw new Error(`打卡失败: ${error.message}`);
  }
  
  return data;
}

// 使用示例
try {
  const attendanceId = await clockIn('仓库ID', '正常上班');
  console.log('打卡成功，记录ID:', attendanceId);
  Taro.showToast({
    title: '打卡成功',
    icon: 'success'
  });
} catch (error) {
  console.error(error);
  Taro.showToast({
    title: error.message,
    icon: 'none'
  });
}
```

---

### 2. 打卡下班

```typescript
async function clockOut(notes?: string) {
  const { data: { user } } = await supabase.auth.getUser();
  
  if (!user) {
    throw new Error('用户未登录');
  }
  
  const { data, error } = await supabase
    .rpc('clock_out', {
      p_user_id: user.id,
      p_notes: notes || null
    });
  
  if (error) {
    throw new Error(`打卡失败: ${error.message}`);
  }
  
  return data;
}

// 使用示例
try {
  await clockOut('正常下班');
  console.log('打卡下班成功');
  Taro.showToast({
    title: '打卡下班成功',
    icon: 'success'
  });
} catch (error) {
  console.error(error);
  Taro.showToast({
    title: error.message,
    icon: 'none'
  });
}
```

---

### 3. 获取今天的考勤状态

```typescript
async function getTodayAttendanceStatus() {
  const { data: { user } } = await supabase.auth.getUser();
  
  if (!user) {
    return null;
  }
  
  const { data, error } = await supabase
    .rpc('get_today_attendance_status', {
      p_user_id: user.id
    });
  
  if (error) {
    console.error('获取考勤状态失败:', error);
    return null;
  }
  
  return data[0];
}

// 使用示例
const status = await getTodayAttendanceStatus();
if (status) {
  console.log('今天是否已打卡上班:', status.has_clocked_in);
  console.log('今天是否已打卡下班:', status.has_clocked_out);
  console.log('上班时间:', status.clock_in_time);
  console.log('下班时间:', status.clock_out_time);
  console.log('工作时长:', status.work_hours);
}
```

---

### 4. 获取用户的考勤记录

```typescript
async function getUserAttendance(startDate?: string, endDate?: string) {
  const { data: { user } } = await supabase.auth.getUser();
  
  if (!user) {
    return [];
  }
  
  const { data, error } = await supabase
    .rpc('get_user_attendance', {
      p_user_id: user.id,
      p_start_date: startDate || null,
      p_end_date: endDate || null
    });
  
  if (error) {
    console.error('获取考勤记录失败:', error);
    return [];
  }
  
  return data;
}

// 使用示例
const records = await getUserAttendance('2025-12-01', '2025-12-31');
console.log('本月考勤记录:', records);
```

---

### 5. 获取考勤统计

```typescript
async function getAttendanceStatistics(startDate: string, endDate: string) {
  const { data: { user } } = await supabase.auth.getUser();
  
  if (!user) {
    return null;
  }
  
  const { data, error } = await supabase
    .rpc('get_attendance_statistics', {
      p_user_id: user.id,
      p_start_date: startDate,
      p_end_date: endDate
    });
  
  if (error) {
    console.error('获取考勤统计失败:', error);
    return null;
  }
  
  return data[0];
}

// 使用示例
const stats = await getAttendanceStatistics('2025-12-01', '2025-12-31');
if (stats) {
  console.log('总天数:', stats.total_days);
  console.log('出勤天数:', stats.present_days);
  console.log('缺勤天数:', stats.absent_days);
  console.log('总工作时长:', stats.total_work_hours);
  console.log('平均工作时长:', stats.average_work_hours);
}
```

---

### 6. 管理员获取所有用户的考勤记录

```typescript
async function getAllAttendance(startDate?: string, endDate?: string) {
  const { data: { user } } = await supabase.auth.getUser();
  
  if (!user) {
    throw new Error('用户未登录');
  }
  
  const { data, error } = await supabase
    .rpc('get_all_attendance', {
      p_admin_id: user.id,
      p_start_date: startDate || null,
      p_end_date: endDate || null
    });
  
  if (error) {
    throw new Error(`获取考勤记录失败: ${error.message}`);
  }
  
  return data;
}

// 使用示例
try {
  const allRecords = await getAllAttendance('2025-12-01', '2025-12-31');
  console.log('所有用户的考勤记录:', allRecords);
} catch (error) {
  console.error(error);
  Taro.showToast({
    title: error.message,
    icon: 'none'
  });
}
```

---

## 📱 完整的考勤管理页面示例

### 司机考勤页面

```typescript
import React, { useEffect, useState } from 'react';
import { View, Text, Button } from '@tarojs/components';
import Taro from '@tarojs/taro';
import { supabase } from '@/client/supabase';

interface AttendanceStatus {
  has_clocked_in: boolean;
  has_clocked_out: boolean;
  clock_in_time: string | null;
  clock_out_time: string | null;
  work_hours: number | null;
}

const DriverAttendance: React.FC = () => {
  const [status, setStatus] = useState<AttendanceStatus | null>(null);
  const [loading, setLoading] = useState(true);
  
  useEffect(() => {
    loadTodayStatus();
  }, []);
  
  const loadTodayStatus = async () => {
    try {
      setLoading(true);
      const { data: { user } } = await supabase.auth.getUser();
      
      if (!user) {
        Taro.showToast({
          title: '请先登录',
          icon: 'none'
        });
        return;
      }
      
      const { data, error } = await supabase
        .rpc('get_today_attendance_status', {
          p_user_id: user.id
        });
      
      if (error) throw error;
      
      setStatus(data[0]);
    } catch (error) {
      console.error('加载考勤状态失败:', error);
      Taro.showToast({
        title: '加载失败',
        icon: 'none'
      });
    } finally {
      setLoading(false);
    }
  };
  
  const handleClockIn = async () => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      
      if (!user) {
        Taro.showToast({
          title: '请先登录',
          icon: 'none'
        });
        return;
      }
      
      const { error } = await supabase
        .rpc('clock_in', {
          p_user_id: user.id
        });
      
      if (error) throw error;
      
      Taro.showToast({
        title: '打卡成功',
        icon: 'success'
      });
      
      // 重新加载状态
      loadTodayStatus();
    } catch (error) {
      console.error('打卡失败:', error);
      Taro.showToast({
        title: error.message || '打卡失败',
        icon: 'none'
      });
    }
  };
  
  const handleClockOut = async () => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      
      if (!user) {
        Taro.showToast({
          title: '请先登录',
          icon: 'none'
        });
        return;
      }
      
      const { error } = await supabase
        .rpc('clock_out', {
          p_user_id: user.id
        });
      
      if (error) throw error;
      
      Taro.showToast({
        title: '打卡下班成功',
        icon: 'success'
      });
      
      // 重新加载状态
      loadTodayStatus();
    } catch (error) {
      console.error('打卡下班失败:', error);
      Taro.showToast({
        title: error.message || '打卡下班失败',
        icon: 'none'
      });
    }
  };
  
  return (
    <View className="p-4">
      <Text className="text-2xl font-bold mb-4">今日考勤</Text>
      
      {loading ? (
        <Text>加载中...</Text>
      ) : status ? (
        <View className="bg-card p-4 rounded-lg">
          {!status.has_clocked_in ? (
            <View>
              <Text className="text-lg mb-4">今天还没有打卡</Text>
              <Button
                className="w-full bg-primary text-white py-4 rounded"
                onClick={handleClockIn}
              >
                打卡上班
              </Button>
            </View>
          ) : !status.has_clocked_out ? (
            <View>
              <Text className="text-lg mb-2">上班时间</Text>
              <Text className="text-muted mb-4">
                {new Date(status.clock_in_time!).toLocaleTimeString()}
              </Text>
              <Button
                className="w-full bg-secondary text-white py-4 rounded"
                onClick={handleClockOut}
              >
                打卡下班
              </Button>
            </View>
          ) : (
            <View>
              <Text className="text-lg mb-2">今日考勤已完成</Text>
              <Text className="text-muted">
                上班时间: {new Date(status.clock_in_time!).toLocaleTimeString()}
              </Text>
              <Text className="text-muted">
                下班时间: {new Date(status.clock_out_time!).toLocaleTimeString()}
              </Text>
              <Text className="text-muted">
                工作时长: {status.work_hours?.toFixed(2)} 小时
              </Text>
            </View>
          )}
        </View>
      ) : null}
    </View>
  );
};

export default DriverAttendance;
```

---

### 管理员考勤管理页面

```typescript
import React, { useEffect, useState } from 'react';
import { View, Text } from '@tarojs/components';
import Taro from '@tarojs/taro';
import { supabase } from '@/client/supabase';

interface AttendanceRecord {
  id: string;
  user_name: string;
  warehouse_name: string;
  clock_in_time: string;
  clock_out_time: string | null;
  work_hours: number | null;
  status: string;
}

const AdminAttendance: React.FC = () => {
  const [records, setRecords] = useState<AttendanceRecord[]>([]);
  const [loading, setLoading] = useState(true);
  
  useEffect(() => {
    loadAllAttendance();
  }, []);
  
  const loadAllAttendance = async () => {
    try {
      setLoading(true);
      const { data: { user } } = await supabase.auth.getUser();
      
      if (!user) {
        Taro.showToast({
          title: '请先登录',
          icon: 'none'
        });
        return;
      }
      
      const { data, error } = await supabase
        .rpc('get_all_attendance', {
          p_admin_id: user.id,
          p_start_date: new Date().toISOString().split('T')[0],
          p_end_date: new Date().toISOString().split('T')[0]
        });
      
      if (error) throw error;
      
      setRecords(data || []);
    } catch (error) {
      console.error('加载考勤记录失败:', error);
      Taro.showToast({
        title: '加载失败',
        icon: 'none'
      });
    } finally {
      setLoading(false);
    }
  };
  
  return (
    <View className="p-4">
      <Text className="text-2xl font-bold mb-4">今日考勤</Text>
      
      {loading ? (
        <Text>加载中...</Text>
      ) : records.length === 0 ? (
        <Text>暂无考勤记录</Text>
      ) : (
        records.map(record => (
          <View key={record.id} className="bg-card p-4 rounded-lg mb-2">
            <Text className="text-lg font-semibold">{record.user_name}</Text>
            <Text className="text-sm text-muted">{record.warehouse_name}</Text>
            <Text className="text-sm">
              上班: {new Date(record.clock_in_time).toLocaleTimeString()}
            </Text>
            {record.clock_out_time && (
              <Text className="text-sm">
                下班: {new Date(record.clock_out_time).toLocaleTimeString()}
              </Text>
            )}
            {record.work_hours && (
              <Text className="text-sm">
                工作时长: {record.work_hours.toFixed(2)} 小时
              </Text>
            )}
          </View>
        ))
      )}
    </View>
  );
};

export default AdminAttendance;
```

---

## 🧪 测试用例

### 测试 1：司机打卡上班

```sql
-- 司机打卡上班
SELECT clock_in('司机ID', '仓库ID', '正常上班');

-- 预期结果：返回考勤记录 ID
```

### 测试 2：司机打卡下班

```sql
-- 司机打卡下班
SELECT clock_out('司机ID', '正常下班');

-- 预期结果：返回 true
```

### 测试 3：获取今天的考勤状态

```sql
-- 获取今天的考勤状态
SELECT * FROM get_today_attendance_status('司机ID');

-- 预期结果：返回今天的考勤状态
```

### 测试 4：获取考勤统计

```sql
-- 获取本月的考勤统计
SELECT * FROM get_attendance_statistics(
  '司机ID',
  '2025-12-01',
  '2025-12-31'
);

-- 预期结果：返回考勤统计数据
```

### 测试 5：管理员查看所有考勤记录

```sql
-- 管理员查看今天所有考勤记录
SELECT * FROM get_all_attendance(
  '管理员ID',
  CURRENT_DATE,
  CURRENT_DATE
);

-- 预期结果：返回所有用户的考勤记录
```

---

## ⚠️ 注意事项

### 1. 打卡限制

- 每天只能打卡上班一次
- 必须先打卡上班才能打卡下班
- 打卡下班后不能再次打卡

### 2. 权限检查

- 司机只能查看和管理自己的考勤记录
- 司机只能更新未完成的考勤记录（未打卡下班）
- 管理员可以查看和管理所有考勤记录

### 3. 数据完整性

- 考勤记录必须关联用户
- 工作时长自动计算
- 考勤状态默认为 'present'

### 4. 性能优化

- 建议添加索引：
  ```sql
  CREATE INDEX IF NOT EXISTS idx_attendance_user_id 
    ON attendance(user_id);
  CREATE INDEX IF NOT EXISTS idx_attendance_work_date 
    ON attendance(work_date);
  CREATE INDEX IF NOT EXISTS idx_attendance_user_date 
    ON attendance(user_id, work_date);
  ```

---

## 📚 相关文档

- [权限系统重构完成报告](./权限系统重构完成报告.md)
- [权限系统测试指南](./测试权限系统.md)
- [权限系统重构说明](./权限系统重构说明.md)

---

**文档版本**: 1.0  
**创建时间**: 2025-12-01  
**适用范围**: 车队管家小程序考勤管理功能  
**状态**: 已完成
