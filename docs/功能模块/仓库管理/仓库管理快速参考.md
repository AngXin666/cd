# 仓库管理快速参考

## 🚀 常用函数

### 1. 获取用户可访问的仓库列表

```typescript
const { data, error } = await supabase
  .rpc('get_user_accessible_warehouses', {
    p_user_id: user.id
  });
```

**返回**: `{ warehouse_id, warehouse_name, is_active }`

---

### 2. 检查用户是否可以访问仓库

```typescript
const { data, error } = await supabase
  .rpc('can_user_access_warehouse', {
    p_user_id: user.id,
    p_warehouse_id: warehouseId
  });
```

**返回**: `boolean`

---

### 3. 获取仓库的用户列表

```typescript
const { data, error } = await supabase
  .rpc('get_warehouse_users', {
    p_warehouse_id: warehouseId
  });
```

**返回**: `{ user_id, user_name, user_email, assigned_at }`

---

### 4. 为用户分配仓库

```typescript
const { data, error } = await supabase
  .rpc('assign_user_to_warehouse', {
    p_user_id: userId,
    p_warehouse_id: warehouseId,
    p_assigned_by: currentUser.id
  });
```

**返回**: `uuid` (分配记录 ID)

**异常**:
- 只有管理员可以分配仓库
- 用户不存在
- 仓库不存在
- 用户已经被分配到该仓库

---

### 5. 取消用户的仓库分配

```typescript
const { data, error } = await supabase
  .rpc('unassign_user_from_warehouse', {
    p_user_id: userId,
    p_warehouse_id: warehouseId,
    p_unassigned_by: currentUser.id
  });
```

**返回**: `boolean`

**异常**:
- 只有管理员可以取消仓库分配
- 分配记录不存在

---

### 6. 批量分配用户到仓库

```typescript
const { data, error } = await supabase
  .rpc('batch_assign_users_to_warehouse', {
    p_user_ids: [userId1, userId2, userId3],
    p_warehouse_id: warehouseId,
    p_assigned_by: currentUser.id
  });
```

**返回**: `{ user_id, success, error_message }[]`

---

## 🔐 权限矩阵

| 操作 | BOSS | MANAGER | DRIVER |
|-----|------|---------|--------|
| 查看所有仓库 | ✅ | ✅ | ❌ |
| 查看自己的仓库 | ✅ | ✅ | ✅ |
| 创建仓库 | ✅ | ✅ | ❌ |
| 更新仓库 | ✅ | ✅ | ❌ |
| 删除仓库 | ✅ | ✅ | ❌ |
| 分配用户到仓库 | ✅ | ✅ | ❌ |
| 取消用户分配 | ✅ | ✅ | ❌ |

---

## 📝 完整示例

### 仓库列表页面

```typescript
import React, { useEffect, useState } from 'react';
import { View, Text } from '@tarojs/components';
import Taro from '@tarojs/taro';
import { supabase } from '@/client/supabase';

const WarehouseList: React.FC = () => {
  const [warehouses, setWarehouses] = useState([]);
  
  useEffect(() => {
    loadWarehouses();
  }, []);
  
  const loadWarehouses = async () => {
    const { data: { user } } = await supabase.auth.getUser();
    
    const { data, error } = await supabase
      .rpc('get_user_accessible_warehouses', {
        p_user_id: user.id
      });
    
    if (!error) {
      setWarehouses(data || []);
    }
  };
  
  return (
    <View className="p-4">
      {warehouses.map(w => (
        <View key={w.warehouse_id} className="bg-card p-4 rounded-lg mb-2">
          <Text className="text-lg font-semibold">{w.warehouse_name}</Text>
        </View>
      ))}
    </View>
  );
};
```

### 分配用户到仓库

```typescript
const handleAssign = async () => {
  try {
    const { data: { user } } = await supabase.auth.getUser();
    
    await supabase.rpc('assign_user_to_warehouse', {
      p_user_id: selectedUserId,
      p_warehouse_id: warehouseId,
      p_assigned_by: user.id
    });
    
    Taro.showToast({
      title: '分配成功',
      icon: 'success'
    });
  } catch (error) {
    Taro.showToast({
      title: error.message,
      icon: 'none'
    });
  }
};
```

---

## 🧪 测试命令

### 验证策略

```sql
-- 验证 warehouses 表策略
SELECT * FROM verify_warehouses_table_policies();

-- 验证 warehouse_assignments 表策略
SELECT * FROM verify_warehouse_assignments_table_policies();
```

### 测试权限

```sql
-- 测试管理员权限
SELECT * FROM warehouses; -- 应该返回所有仓库

-- 测试司机权限
SELECT * FROM warehouses; -- 应该只返回自己的仓库
```

---

## 📚 相关文档

- [仓库管理功能使用指南](./仓库管理功能使用指南.md) - 完整文档
- [仓库管理RLS策略应用总结](./仓库管理RLS策略应用总结.md) - 实施总结
- [权限系统重构完成报告](./权限系统重构完成报告.md) - 重构报告

---

**版本**: 1.0  
**日期**: 2025-12-01
