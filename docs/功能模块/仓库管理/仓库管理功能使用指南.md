# 仓库管理功能使用指南

## 📋 概述

本文档介绍了车队管家小程序中仓库管理功能的使用方法，包括权限设计、API 使用和前端集成。

---

## 🔐 权限设计

### 角色权限矩阵

| 操作 | BOSS | MANAGER | DRIVER |
|-----|------|---------|--------|
| 查看所有仓库 | ✅ | ✅ | ❌ |
| 查看自己的仓库 | ✅ | ✅ | ✅ |
| 创建仓库 | ✅ | ✅ | ❌ |
| 更新仓库 | ✅ | ✅ | ❌ |
| 删除仓库 | ✅ | ✅ | ❌ |
| 分配用户到仓库 | ✅ | ✅ | ❌ |
| 取消用户分配 | ✅ | ✅ | ❌ |

### RLS 策略说明

#### warehouses 表策略

1. **new_admins_view_all_warehouses** (SELECT)
   - 管理员可以查看所有仓库
   - 规则: `is_admin(auth.uid())`

2. **new_drivers_view_assigned_warehouses** (SELECT)
   - 司机可以查看自己被分配的仓库
   - 规则: `EXISTS (SELECT 1 FROM warehouse_assignments WHERE warehouse_id = warehouses.id AND user_id = auth.uid())`

3. **new_admins_insert_warehouses** (INSERT)
   - 管理员可以创建仓库
   - 规则: `is_admin(auth.uid())`

4. **new_admins_update_all_warehouses** (UPDATE)
   - 管理员可以更新所有仓库
   - 规则: `is_admin(auth.uid())`

5. **new_admins_delete_warehouses** (DELETE)
   - 管理员可以删除仓库
   - 规则: `is_admin(auth.uid())`

#### warehouse_assignments 表策略

1. **new_admins_view_all_warehouse_assignments** (SELECT)
   - 管理员可以查看所有仓库分配
   - 规则: `is_admin(auth.uid())`

2. **new_users_view_own_warehouse_assignments** (SELECT)
   - 用户可以查看自己的仓库分配
   - 规则: `user_id = auth.uid()`

3. **new_admins_insert_warehouse_assignments** (INSERT)
   - 管理员可以创建仓库分配
   - 规则: `is_admin(auth.uid())`

4. **new_admins_update_all_warehouse_assignments** (UPDATE)
   - 管理员可以更新所有仓库分配
   - 规则: `is_admin(auth.uid())`

5. **new_admins_delete_warehouse_assignments** (DELETE)
   - 管理员可以删除仓库分配
   - 规则: `is_admin(auth.uid())`

---

## 🛠️ 数据库函数

### 1. 检查用户是否可以访问仓库

```sql
can_user_access_warehouse(p_user_id uuid, p_warehouse_id uuid) RETURNS boolean
```

**功能**: 检查用户是否可以访问某个仓库

**参数**:
- `p_user_id`: 用户 ID
- `p_warehouse_id`: 仓库 ID

**返回**: 布尔值，true 表示可以访问

**示例**:
```sql
SELECT can_user_access_warehouse(
  '用户ID',
  '仓库ID'
);
```

### 2. 获取用户可访问的仓库列表

```sql
get_user_accessible_warehouses(p_user_id uuid) 
RETURNS TABLE (warehouse_id uuid, warehouse_name text, is_active boolean)
```

**功能**: 获取用户可访问的所有仓库

**参数**:
- `p_user_id`: 用户 ID

**返回**: 仓库列表（ID、名称、是否激活）

**示例**:
```sql
SELECT * FROM get_user_accessible_warehouses('用户ID');
```

### 3. 获取仓库的用户列表

```sql
get_warehouse_users(p_warehouse_id uuid)
RETURNS TABLE (user_id uuid, user_name text, user_email text, assigned_at timestamptz)
```

**功能**: 获取分配到某个仓库的所有用户

**参数**:
- `p_warehouse_id`: 仓库 ID

**返回**: 用户列表（ID、姓名、邮箱、分配时间）

**示例**:
```sql
SELECT * FROM get_warehouse_users('仓库ID');
```

### 4. 为用户分配仓库

```sql
assign_user_to_warehouse(p_user_id uuid, p_warehouse_id uuid, p_assigned_by uuid)
RETURNS uuid
```

**功能**: 为用户分配仓库

**参数**:
- `p_user_id`: 用户 ID
- `p_warehouse_id`: 仓库 ID
- `p_assigned_by`: 分配者 ID（必须是管理员）

**返回**: 分配记录 ID

**异常**:
- 只有管理员可以分配仓库
- 用户不存在
- 仓库不存在
- 用户已经被分配到该仓库

**示例**:
```sql
SELECT assign_user_to_warehouse(
  '用户ID',
  '仓库ID',
  '管理员ID'
);
```

### 5. 取消用户的仓库分配

```sql
unassign_user_from_warehouse(p_user_id uuid, p_warehouse_id uuid, p_unassigned_by uuid)
RETURNS boolean
```

**功能**: 取消用户的仓库分配

**参数**:
- `p_user_id`: 用户 ID
- `p_warehouse_id`: 仓库 ID
- `p_unassigned_by`: 取消分配者 ID（必须是管理员）

**返回**: 布尔值，true 表示成功

**异常**:
- 只有管理员可以取消仓库分配
- 分配记录不存在

**示例**:
```sql
SELECT unassign_user_from_warehouse(
  '用户ID',
  '仓库ID',
  '管理员ID'
);
```

### 6. 批量分配用户到仓库

```sql
batch_assign_users_to_warehouse(p_user_ids uuid[], p_warehouse_id uuid, p_assigned_by uuid)
RETURNS TABLE (user_id uuid, success boolean, error_message text)
```

**功能**: 批量为多个用户分配同一个仓库

**参数**:
- `p_user_ids`: 用户 ID 数组
- `p_warehouse_id`: 仓库 ID
- `p_assigned_by`: 分配者 ID（必须是管理员）

**返回**: 每个用户的分配结果（用户 ID、是否成功、错误信息）

**示例**:
```sql
SELECT * FROM batch_assign_users_to_warehouse(
  ARRAY['用户ID1', '用户ID2', '用户ID3'],
  '仓库ID',
  '管理员ID'
);
```

---

## 💻 前端集成

### 1. 获取用户可访问的仓库列表

```typescript
import { supabase } from '@/client/supabase';

async function getUserWarehouses() {
  const { data: { user } } = await supabase.auth.getUser();
  
  if (!user) {
    console.error('用户未登录');
    return [];
  }
  
  const { data, error } = await supabase
    .rpc('get_user_accessible_warehouses', {
      p_user_id: user.id
    });
  
  if (error) {
    console.error('获取仓库列表失败:', error);
    return [];
  }
  
  return data;
}

// 使用示例
const warehouses = await getUserWarehouses();
console.log('可访问的仓库:', warehouses);
```

### 2. 检查用户是否可以访问仓库

```typescript
async function checkWarehouseAccess(warehouseId: string) {
  const { data: { user } } = await supabase.auth.getUser();
  
  if (!user) {
    return false;
  }
  
  const { data, error } = await supabase
    .rpc('can_user_access_warehouse', {
      p_user_id: user.id,
      p_warehouse_id: warehouseId
    });
  
  if (error) {
    console.error('检查仓库访问权限失败:', error);
    return false;
  }
  
  return data;
}

// 使用示例
const canAccess = await checkWarehouseAccess('仓库ID');
if (canAccess) {
  console.log('可以访问该仓库');
} else {
  console.log('无权访问该仓库');
}
```

### 3. 获取仓库的用户列表

```typescript
async function getWarehouseUsers(warehouseId: string) {
  const { data, error } = await supabase
    .rpc('get_warehouse_users', {
      p_warehouse_id: warehouseId
    });
  
  if (error) {
    console.error('获取仓库用户列表失败:', error);
    return [];
  }
  
  return data;
}

// 使用示例
const users = await getWarehouseUsers('仓库ID');
console.log('仓库用户列表:', users);
```

### 4. 为用户分配仓库

```typescript
async function assignUserToWarehouse(userId: string, warehouseId: string) {
  const { data: { user } } = await supabase.auth.getUser();
  
  if (!user) {
    throw new Error('用户未登录');
  }
  
  const { data, error } = await supabase
    .rpc('assign_user_to_warehouse', {
      p_user_id: userId,
      p_warehouse_id: warehouseId,
      p_assigned_by: user.id
    });
  
  if (error) {
    throw new Error(`分配仓库失败: ${error.message}`);
  }
  
  return data;
}

// 使用示例
try {
  const assignmentId = await assignUserToWarehouse('用户ID', '仓库ID');
  console.log('分配成功，记录ID:', assignmentId);
  Taro.showToast({
    title: '分配成功',
    icon: 'success'
  });
} catch (error) {
  console.error(error);
  Taro.showToast({
    title: error.message,
    icon: 'none'
  });
}
```

### 5. 取消用户的仓库分配

```typescript
async function unassignUserFromWarehouse(userId: string, warehouseId: string) {
  const { data: { user } } = await supabase.auth.getUser();
  
  if (!user) {
    throw new Error('用户未登录');
  }
  
  const { data, error } = await supabase
    .rpc('unassign_user_from_warehouse', {
      p_user_id: userId,
      p_warehouse_id: warehouseId,
      p_unassigned_by: user.id
    });
  
  if (error) {
    throw new Error(`取消分配失败: ${error.message}`);
  }
  
  return data;
}

// 使用示例
try {
  await unassignUserFromWarehouse('用户ID', '仓库ID');
  console.log('取消分配成功');
  Taro.showToast({
    title: '取消分配成功',
    icon: 'success'
  });
} catch (error) {
  console.error(error);
  Taro.showToast({
    title: error.message,
    icon: 'none'
  });
}
```

### 6. 批量分配用户到仓库

```typescript
async function batchAssignUsersToWarehouse(userIds: string[], warehouseId: string) {
  const { data: { user } } = await supabase.auth.getUser();
  
  if (!user) {
    throw new Error('用户未登录');
  }
  
  const { data, error } = await supabase
    .rpc('batch_assign_users_to_warehouse', {
      p_user_ids: userIds,
      p_warehouse_id: warehouseId,
      p_assigned_by: user.id
    });
  
  if (error) {
    throw new Error(`批量分配失败: ${error.message}`);
  }
  
  return data;
}

// 使用示例
try {
  const results = await batchAssignUsersToWarehouse(
    ['用户ID1', '用户ID2', '用户ID3'],
    '仓库ID'
  );
  
  const successCount = results.filter(r => r.success).length;
  const failCount = results.filter(r => !r.success).length;
  
  console.log(`批量分配完成: 成功 ${successCount} 个，失败 ${failCount} 个`);
  
  // 显示失败的记录
  results.filter(r => !r.success).forEach(r => {
    console.error(`用户 ${r.user_id} 分配失败: ${r.error_message}`);
  });
  
  Taro.showToast({
    title: `成功分配 ${successCount} 个用户`,
    icon: 'success'
  });
} catch (error) {
  console.error(error);
  Taro.showToast({
    title: error.message,
    icon: 'none'
  });
}
```

---

## 📱 完整的仓库管理页面示例

### 仓库列表页面

```typescript
import React, { useEffect, useState } from 'react';
import { View, Text, Button } from '@tarojs/components';
import Taro from '@tarojs/taro';
import { supabase } from '@/client/supabase';

interface Warehouse {
  warehouse_id: string;
  warehouse_name: string;
  is_active: boolean;
}

const WarehouseList: React.FC = () => {
  const [warehouses, setWarehouses] = useState<Warehouse[]>([]);
  const [loading, setLoading] = useState(true);
  
  useEffect(() => {
    loadWarehouses();
  }, []);
  
  const loadWarehouses = async () => {
    try {
      setLoading(true);
      const { data: { user } } = await supabase.auth.getUser();
      
      if (!user) {
        Taro.showToast({
          title: '请先登录',
          icon: 'none'
        });
        return;
      }
      
      const { data, error } = await supabase
        .rpc('get_user_accessible_warehouses', {
          p_user_id: user.id
        });
      
      if (error) throw error;
      
      setWarehouses(data || []);
    } catch (error) {
      console.error('加载仓库列表失败:', error);
      Taro.showToast({
        title: '加载失败',
        icon: 'none'
      });
    } finally {
      setLoading(false);
    }
  };
  
  const handleWarehouseClick = (warehouseId: string) => {
    Taro.navigateTo({
      url: `/pages/warehouse/detail/index?id=${warehouseId}`
    });
  };
  
  return (
    <View className="p-4">
      <Text className="text-2xl font-bold mb-4">仓库列表</Text>
      
      {loading ? (
        <Text>加载中...</Text>
      ) : warehouses.length === 0 ? (
        <Text>暂无仓库</Text>
      ) : (
        warehouses.map(warehouse => (
          <View
            key={warehouse.warehouse_id}
            className="bg-card p-4 rounded-lg mb-2"
            onClick={() => handleWarehouseClick(warehouse.warehouse_id)}
          >
            <Text className="text-lg font-semibold">{warehouse.warehouse_name}</Text>
            <Text className={`text-sm ${warehouse.is_active ? 'text-primary' : 'text-muted'}`}>
              {warehouse.is_active ? '激活' : '未激活'}
            </Text>
          </View>
        ))
      )}
    </View>
  );
};

export default WarehouseList;
```

### 仓库详情页面

```typescript
import React, { useEffect, useState } from 'react';
import { View, Text, Button } from '@tarojs/components';
import Taro, { useRouter } from '@tarojs/taro';
import { supabase } from '@/client/supabase';

interface WarehouseUser {
  user_id: string;
  user_name: string;
  user_email: string;
  assigned_at: string;
}

const WarehouseDetail: React.FC = () => {
  const router = useRouter();
  const warehouseId = router.params.id;
  
  const [users, setUsers] = useState<WarehouseUser[]>([]);
  const [loading, setLoading] = useState(true);
  
  useEffect(() => {
    if (warehouseId) {
      loadWarehouseUsers();
    }
  }, [warehouseId]);
  
  const loadWarehouseUsers = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase
        .rpc('get_warehouse_users', {
          p_warehouse_id: warehouseId
        });
      
      if (error) throw error;
      
      setUsers(data || []);
    } catch (error) {
      console.error('加载仓库用户失败:', error);
      Taro.showToast({
        title: '加载失败',
        icon: 'none'
      });
    } finally {
      setLoading(false);
    }
  };
  
  const handleUnassignUser = async (userId: string) => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      
      if (!user) {
        Taro.showToast({
          title: '请先登录',
          icon: 'none'
        });
        return;
      }
      
      const { error } = await supabase
        .rpc('unassign_user_from_warehouse', {
          p_user_id: userId,
          p_warehouse_id: warehouseId,
          p_unassigned_by: user.id
        });
      
      if (error) throw error;
      
      Taro.showToast({
        title: '取消分配成功',
        icon: 'success'
      });
      
      // 重新加载用户列表
      loadWarehouseUsers();
    } catch (error) {
      console.error('取消分配失败:', error);
      Taro.showToast({
        title: error.message || '取消分配失败',
        icon: 'none'
      });
    }
  };
  
  return (
    <View className="p-4">
      <Text className="text-2xl font-bold mb-4">仓库用户</Text>
      
      {loading ? (
        <Text>加载中...</Text>
      ) : users.length === 0 ? (
        <Text>暂无用户</Text>
      ) : (
        users.map(user => (
          <View key={user.user_id} className="bg-card p-4 rounded-lg mb-2">
            <Text className="text-lg font-semibold">{user.user_name}</Text>
            <Text className="text-sm text-muted">{user.user_email}</Text>
            <Text className="text-xs text-muted">
              分配时间: {new Date(user.assigned_at).toLocaleString()}
            </Text>
            <Button
              className="mt-2"
              size="mini"
              onClick={() => handleUnassignUser(user.user_id)}
            >
              取消分配
            </Button>
          </View>
        ))
      )}
    </View>
  );
};

export default WarehouseDetail;
```

---

## 🧪 测试用例

### 测试 1：管理员查看所有仓库

```sql
-- 假设管理员 ID 为 'admin-uuid'
-- 设置会话用户
SET LOCAL "request.jwt.claims" = '{"sub": "admin-uuid"}';

-- 查询所有仓库
SELECT * FROM warehouses;

-- 预期结果：返回所有仓库
```

### 测试 2：司机查看自己的仓库

```sql
-- 假设司机 ID 为 'driver-uuid'
-- 设置会话用户
SET LOCAL "request.jwt.claims" = '{"sub": "driver-uuid"}';

-- 查询仓库
SELECT * FROM warehouses;

-- 预期结果：只返回该司机被分配的仓库
```

### 测试 3：为用户分配仓库

```sql
-- 管理员为司机分配仓库
SELECT assign_user_to_warehouse(
  'driver-uuid',
  'warehouse-uuid',
  'admin-uuid'
);

-- 预期结果：返回分配记录 ID
```

### 测试 4：取消用户的仓库分配

```sql
-- 管理员取消司机的仓库分配
SELECT unassign_user_from_warehouse(
  'driver-uuid',
  'warehouse-uuid',
  'admin-uuid'
);

-- 预期结果：返回 true
```

### 测试 5：批量分配用户到仓库

```sql
-- 管理员批量分配多个司机到仓库
SELECT * FROM batch_assign_users_to_warehouse(
  ARRAY['driver-uuid-1', 'driver-uuid-2', 'driver-uuid-3'],
  'warehouse-uuid',
  'admin-uuid'
);

-- 预期结果：返回每个用户的分配结果
```

---

## ⚠️ 注意事项

### 1. 权限检查

- 所有仓库管理操作都需要管理员权限
- 司机只能查看自己被分配的仓库
- 在前端调用函数前，建议先检查用户角色

### 2. 错误处理

- 所有数据库函数都会抛出异常，需要在前端捕获并处理
- 建议使用 try-catch 包裹所有数据库调用
- 向用户显示友好的错误信息

### 3. 性能优化

- 仓库列表建议添加分页
- 用户列表建议添加搜索和筛选功能
- 考虑缓存常用的仓库列表

### 4. 数据一致性

- 删除仓库前，建议先检查是否有用户分配
- 删除用户前，建议先取消所有仓库分配
- 使用事务确保数据一致性

---

## 📚 相关文档

- [权限系统重构完成报告](./权限系统重构完成报告.md)
- [权限系统测试指南](./测试权限系统.md)
- [权限系统重构说明](./权限系统重构说明.md)

---

**文档版本**: 1.0  
**创建时间**: 2025-12-01  
**适用范围**: 车队管家小程序仓库管理功能  
**状态**: 已完成
