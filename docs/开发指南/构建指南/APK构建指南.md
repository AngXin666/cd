# 车队管家 APK 构建指南

## 📱 快速构建APK（5分钟完成）

### 方法一：自动构建（推荐）⭐

#### 步骤1：安装Java JDK 17
1. 打开下载页面：https://adoptium.net/zh-CN/temurin/releases/
2. 选择：
   - 版本：17 - LTS
   - 操作系统：Windows
   - 架构：x64
   - 包类型：JDK
   - 格式：MSI
3. 下载并双击安装（使用默认设置即可）

#### 步骤2：运行自动构建脚本
打开PowerShell（在项目目录右键 -> "在终端中打开"），运行：
```powershell
.\build-apk.ps1
```

**就这么简单！** 脚本会自动：
- ✅ 检查Java环境
- ✅ 构建H5项目
- ✅ 同步到Android
- ✅ 构建APK
- ✅ 自动打开APK文件位置

构建完成后，APK会保存在：`VehicleManager.apk`

---

### 方法二：使用Android Studio（图形化）

#### 步骤1：安装环境
- Java JDK 17（同方法一）
- Android Studio：https://developer.android.com/studio

#### 步骤2：打开项目
```powershell
npx cap open android
```

#### 步骤3：在Android Studio中构建
1. 等待Gradle同步完成
2. 点击菜单：Build → Build Bundle(s) / APK(s) → Build APK(s)
3. 等待构建完成（约2-5分钟）
4. 点击弹窗中的 "locate" 查看APK位置

APK位置：`android\app\build\outputs\apk\debug\app-debug.apk`

---

### 方法三：手动命令行构建

#### 前提条件
- 已安装Java JDK 17
- 已配置JAVA_HOME环境变量

#### 构建步骤
```powershell
# 1. 构建H5项目
pnpm run build:h5

# 2. 同步到Android
npx cap sync android

# 3. 构建APK
cd android
.\gradlew assembleDebug

# 4. APK位置
# android\app\build\outputs\apk\debug\app-debug.apk
```

---

## 🔧 环境配置说明

### Java环境变量（通常安装时自动配置）
如需手动配置：
```
JAVA_HOME = C:\Program Files\Eclipse Adoptium\jdk-17.0.9.9-hotspot
Path = %JAVA_HOME%\bin;其他路径...
```

### 验证Java安装
```powershell
java -version
```
应显示：`openjdk version "17.0.x"`

---

## 📦 构建产物说明

### Debug版本（开发测试用）
- 文件名：`app-debug.apk`  
- 签名：使用debug签名
- 大小：约25-35MB
- 用途：内部测试

### Release版本（生产发布用）
需要配置签名证书后使用：
```powershell
.\gradlew assembleRelease
```

---

## ❓ 常见问题

### Q1：JAVA_HOME未设置
**错误**：`ERROR: JAVA_HOME is not set`
**解决**：重新安装Java JDK 17，或手动配置环境变量

### Q2：Gradle下载慢
**解决**：配置国内镜像源，编辑 `android/build.gradle`

### Q3：构建失败
**检查**：
1. Java版本是否为17
2. 是否已运行`pnpm run build:h5`
3. 是否已运行`npx cap sync android`

---

## 📝 当前状态

✅ **已完成**：
- H5项目已构建
- 代码已同步到Android项目  
- 构建脚本已创建

⏳ **待完成**：
- 安装Java JDK 17
- 运行构建脚本

---

## 💡 提示

- 首次构建会下载Gradle依赖，需要5-10分钟
- 后续构建只需1-2分钟
- APK可直接安装到Android手机测试
- 记得开启"允许安装未知来源应用"

---

## 📞 需要帮助？

如遇到问题，请提供：
1. 错误信息截图
2. Java版本（`java -version`）
3. 构建日志

祝构建顺利！🎉
