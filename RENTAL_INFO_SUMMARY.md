...
1. 第一次获取路由参数并设置到 state
2. 第二次检查 state 并调用加载函数

由于 `setState` 是异步的，第二次 `useDidShow` 执行时 state 可能还未更新，导致加载函数直接返回。

**解决方案**：
将两次 `useDidShow` 合并为一次，直接在获取路由参数后立即调用 `loadVehicle(id)`，不依赖 state 的更新。

**修改内容**：
1. 将 `loadVehicle` 函数改为接收 `id` 参数
2. 移除 `vehicleId` state
...
**相关文档**：
- `/RENTAL_EDIT_FIX.md` - 详细的问题分析和修复说明
- `/RENTAL_FEATURE_TEST.md` - 完整的功能测试清单
