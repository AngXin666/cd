...
为老板端考勤管理模块添加计件统计页面。该页面与车队长端计件统计页面结构一致，使用相同的司机卡片布局，显示计件统计（今日数量、本周数量、本月数量）。老板可以查看所有仓库的司机计件数据，支持按仓库切换查看。
...
**User Story:** As a 老板, I want to 在考勤管理中访问计件统计页面, so that I can 查看所有司机的计件数据。

#### Acceptance Criteria

1. WHEN 老板进入考勤管理页面 THEN THE Piece_Work_Page SHALL 作为新的标签页显示在"考勤记录"和"请假审批"之间
2. THE 标签页 SHALL 显示图标"📊"和文字"计件统计"
3. WHEN 点击计件统计标签 THEN THE Piece_Work_Page SHALL 切换显示计件统计内容

### Requirement 2: 司机卡片显示

**User Story:** As a 老板, I want to 查看每个司机的计件统计, so that I can 了解司机的工作量。
...
**User Story:** As a 老板, I want to 查看司机的今日、本周、本月计件数量, so that I can 评估司机工作表现。
...
**User Story:** As a 老板, I want to 按仓库切换查看司机计件数据, so that I can 分别查看各仓库的工作情况。

#### Acceptance Criteria

1. WHEN 存在多个仓库 THEN THE Piece_Work_Page SHALL 显示仓库切换器
2. THE 仓库切换器 SHALL 使用 swiper 组件实现左右滑动切换
3. WHEN 切换仓库 THEN THE Piece_Work_Page SHALL 只显示该仓库的司机
4. THE 仓库切换器 SHALL 显示当前仓库名称和司机数量

### Requirement 5: 数据筛选

**User Story:** As a 老板, I want to 搜索特定司机的计件数据, so that I can 快速找到目标司机。

#### Acceptance Criteria

1. THE 搜索功能 SHALL 支持按姓名、手机号、拼音首字母搜索
2. WHEN 搜索框为空 THEN THE Piece_Work_Page SHALL 显示当前仓库的所有司机

### Requirement 6: 操作按钮

**User Story:** As a 老板, I want to 从计件页面快速访问司机详情, so that I can 查看更多信息。
...
**User Story:** As a 老板, I want to 看到司机总数统计, so that I can 了解当前仓库的司机规模。
