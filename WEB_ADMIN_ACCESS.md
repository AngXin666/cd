# 电脑端管理后台访问说明

## 📍 访问地址

```
https://app.appmiaoda.com/app-7cdqf07mbu9t/#/pages/web-admin/index
```

---

## 🔐 权限说明

### ✅ 允许访问的角色
- **超级管理员（老板）**：13800000001 / 123456
- **管理员（车队长）**：13800000002 / 123456

### ❌ 拒绝访问的角色
- **司机**：13800000003、13800000004
  - 司机尝试访问时会看到"无权限访问，仅限管理员使用"提示
  - 2秒后自动跳转回首页

---

## 🎯 当前功能

### 已实现
- ✅ **权限验证**：自动检查用户角色，仅允许管理员和老板访问
- ✅ **用户信息展示**：显示当前登录用户的姓名、手机号、角色
- ✅ **退出登录**：点击右上角"退出登录"按钮可安全退出
- ✅ **响应式设计**：适配电脑、平板、手机等不同屏幕尺寸
- ✅ **功能预告**：展示即将上线的功能模块

### 开发中
- 🚧 仓库配置管理
- 🚧 用户管理
- 🚧 件数报表
- 🚧 考勤管理

---

## 📱 使用步骤

### 1. 打开浏览器
推荐使用以下浏览器访问：
- Chrome（推荐）
- Edge
- Safari
- Firefox

### 2. 输入访问地址
```
https://app.appmiaoda.com/app-7cdqf07mbu9t/#/pages/web-admin/index
```

### 3. 登录账号
- 如果未登录，系统会自动跳转到登录页面
- 使用管理员或老板账号登录：
  - 老板：13800000001 / 123456
  - 车队长：13800000002 / 123456

### 4. 进入管理后台
- 登录成功后，系统会自动验证权限
- 验证通过后，进入管理后台首页
- 可以看到欢迎信息、用户信息和功能预告

---

## 🎨 界面特点

### 顶部导航栏
- 左侧：车队管家 Logo 和标题
- 右侧：用户信息和退出登录按钮

### 欢迎信息
- 个性化欢迎语
- 功能开发提示

### 用户信息卡片
- 姓名
- 手机号
- 角色标签（带颜色区分）

### 功能预告
- 即将上线的功能模块展示
- 卡片式布局，清晰直观

---

## ⚠️ 注意事项

1. **权限限制**
   - 只有管理员和老板可以访问
   - 司机无法访问，会被自动拦截

2. **浏览器兼容性**
   - 推荐使用现代浏览器（Chrome、Edge、Safari、Firefox）
   - 不支持 IE 浏览器

3. **网络要求**
   - 需要稳定的网络连接
   - 首次加载可能需要几秒钟

4. **登录状态**
   - 登录状态会保持一段时间
   - 长时间未操作可能需要重新登录

---

## 🔧 技术实现

### 权限验证机制
1. **登录验证**：通过 `miaoda-auth-taro` 的 `useAuth({guard: true})` 实现
2. **角色验证**：通过 `useAdminAuth` Hook 检查用户角色
3. **自动拦截**：未授权用户显示提示并自动跳转到首页
4. **友好提示**：显示"无权限访问，仅限管理员使用"提示信息

### 核心代码
```typescript
// useAdminAuth Hook
export const useAdminAuth = () => {
  const {user} = useAuth({guard: true})
  const [profile, setProfile] = useState<Profile | null>(null)
  const [isAuthorized, setIsAuthorized] = useState(false)
  const [isLoading, setIsLoading] = useState(true)

  useEffect(() => {
    const checkAuth = async () => {
      if (!user) return
      
      const data = await getCurrentUserProfile()
      setProfile(data)
      
      // 检查权限：只允许管理员和超级管理员访问
      if (data && (data.role === 'manager' || data.role === 'super_admin')) {
        setIsAuthorized(true)
      } else {
        setIsAuthorized(false)
        showToast({title: '无权限访问，仅限管理员使用'})
        setTimeout(() => redirectTo({url: '/pages/index/index'}), 2000)
      }
    }
    
    checkAuth()
  }, [user])

  return {profile, isAuthorized, isLoading}
}
```

---

## 📞 技术支持

如有任何问题或建议，请联系技术支持团队。

---

**创建日期**：2025-11-05  
**更新日期**：2025-11-05  
**版本**：v1.0.0
