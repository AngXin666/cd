# 通知系统问题解决方案

## 🔴 当前问题

根据您提供的日志：
```
⚠️ 未找到主账号
⚠️ 未找到车队长信息
⚠️ 没有找到任何通知接收者，通知发送完成（无接收者）
```

**问题原因**：`user_roles` 表中缺少用户角色数据

## ✅ 解决方案

### 方案 1：自动修复（推荐）

执行以下 SQL 语句，系统会自动为所有用户添加角色：

```sql
-- 1. 为第一个用户添加 SUPER_ADMIN 角色
INSERT INTO user_roles (user_id, role, created_at)
SELECT 
  u.id,
  'SUPER_ADMIN'::user_role,
  NOW()
FROM users u
LEFT JOIN user_roles ur ON u.id = ur.user_id
WHERE ur.role IS NULL
ORDER BY u.created_at
LIMIT 1
ON CONFLICT (user_id) DO NOTHING;

-- 2. 为其他用户添加 DRIVER 角色
INSERT INTO user_roles (user_id, role, created_at)
SELECT 
  u.id,
  'DRIVER'::user_role,
  NOW()
FROM users u
LEFT JOIN user_roles ur ON u.id = ur.user_id
WHERE ur.role IS NULL
ON CONFLICT (user_id) DO NOTHING;
```

### 方案 2：验证修复结果

执行以下 SQL 确认修复成功：

```sql
-- 查看所有用户及其角色
SELECT 
  u.id,
  u.name,
  u.phone,
  ur.role,
  u.created_at
FROM users u
LEFT JOIN user_roles ur ON u.id = ur.user_id
ORDER BY u.created_at;
```

**预期结果**：
- ✅ 第一个用户的角色是 `SUPER_ADMIN`
- ✅ 其他用户的角色是 `DRIVER`
- ✅ 没有用户的角色是 `NULL`

## 📋 后续步骤

### 1. 如果需要车队长功能

将某些用户的角色改为 `MANAGER`：

```sql
-- 查看所有用户
SELECT id, name, phone, ur.role
FROM users u
LEFT JOIN user_roles ur ON u.id = ur.user_id
ORDER BY created_at;

-- 将用户改为 MANAGER（替换 <用户ID>）
UPDATE user_roles
SET role = 'MANAGER'::user_role
WHERE user_id = '<用户ID>';
```

### 2. 为车队长分配仓库

```sql
-- 查看所有仓库
SELECT id, name FROM warehouses;

-- 为车队长分配仓库（替换 <车队长ID> 和 <仓库ID>）
INSERT INTO warehouse_assignments (user_id, warehouse_id, created_at)
VALUES ('<车队长ID>', '<仓库ID>', NOW())
ON CONFLICT (user_id, warehouse_id) DO NOTHING;
```

### 3. 重新测试

1. **刷新页面**
2. **重新登录**
3. **提交请假申请**
4. **查看控制台日志**，应该看到：
   ```
   ✅ 找到主账号
   ✅ 找到有管辖权的车队长
   ✅ 通知发送成功
   ```

## 🔍 检查脚本

如果需要检查当前状态，可以使用以下脚本：

### 检查用户角色
```sql
-- 统计各角色的用户数量
SELECT 
  COALESCE(ur.role::text, '无角色') as role,
  COUNT(*) as count
FROM users u
LEFT JOIN user_roles ur ON u.id = ur.user_id
GROUP BY ur.role;
```

### 检查仓库分配
```sql
-- 查看所有仓库分配
SELECT 
  u.name as user_name,
  ur.role,
  w.name as warehouse_name
FROM warehouse_assignments wa
JOIN users u ON wa.user_id = u.id
JOIN warehouses w ON wa.warehouse_id = w.id
LEFT JOIN user_roles ur ON u.id = ur.user_id
ORDER BY u.name;
```

## 📝 相关文档

- `测试指南.md`：详细的测试步骤
- `检查用户角色.sql`：检查脚本
- `修复用户角色.sql`：修复脚本
- `supabase/migrations/00531_fix_missing_user_roles.sql`：数据库迁移

---

**最后更新**：2025-12-01
**状态**：等待执行修复 SQL
