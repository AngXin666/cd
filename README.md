...
### 多角色权限管理
- **司机端**：个人工作台、**综合数据统计**（当日件数、当日收入、本月件数、本月收入、出勤天数、请假天数）、**快捷功能**（计件录入、考勤打卡、请假申请、数据统计）、仓库打卡、查看所属仓库、仓库统计详情（考勤+计件）、计件录入（录入、编辑、删除个人计件记录）、我的计件（查看个人计件记录和收入）、**快捷请假**（滚轮选择天数）、**补请假**（补录历史记录）、**离职申请**（提前天数控制）、**草稿管理**（保存、编辑、提交、删除草稿）、**智能天数计算**（自动计算请假天数）、**退出登录**（页面底部）
- **普通管理端**：管理员工作台、**滑动切换仓库**（支持左右滑动快速切换管辖仓库，查看不同仓库数据）、司机管理、基础管理功能、数据统计（查看管辖仓库的计件和考勤数据报表，支持按司机和仓库筛选，显示出勤天数、迟到天数、请假天数，仅可查看）、请假离职审批（**仅审批管辖仓库的申请**）
- **超级管理端**：超级管理员控制台、系统管理、**用户权限管理**（用户列表、角色升降级、权限配置）、**仓库信息管理**（统一编辑仓库基本信息、考勤规则、请假与离职设置，所有修改操作需密码验证）、司机仓库分配、管理员仓库分配、计件品类管理、数据统计（查看所有仓库的计件和考勤数据报表，显示出勤天数、迟到天数、请假天数）、请假离职审批（**仅审批未分配管理员的仓库申请**）

### 管理员仓库切换功能（新增）
- **滑动切换**：左右滑动即可快速切换不同仓库
- **位置提示**：显示当前仓库位置，如"(1/3)"
- **智能加载**：切换仓库时优先使用缓存，响应速度 < 500ms
- **实时更新**：司机提交数据后仪表板自动刷新
...
- 普通管理员：可查看所有用户，可修改司机角色，可查看考勤记录，可查看管辖仓库的计件数据（仅可查看，不能修改或删除），权限受超级管理员配置控制
...
- **管辖仓库分配**：
  ...
  - 管理员只能查看和管理被分配的仓库数据
- **功能权限开关**：
  - **用户信息修改权**：控制是否可以编辑司机信息
  - **用户计件数据修改权**：控制是否可以修改司机的计件数据
  - **考勤规则管理权**：控制是否可以修改考勤规则
  - **系统权限设置**：控制是否可以访问系统设置（高级权限）
