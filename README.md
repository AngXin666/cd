...
- ✅ **解决方案**：
  1. **创建租户数据查询 RPC 函数**：
     - `get_tenant_profiles(role_filter)` - 查询租户 Schema 中的用户档案
     - `get_tenant_drivers()` - 查询租户 Schema 中的司机列表
     - `get_tenant_profile_by_id(user_id)` - 查询租户 Schema 中的指定用户档案
     - `get_all_tenant_profiles()` - 查询租户 Schema 中的所有用户
  2. **使用 SECURITY DEFINER 权限**：
     - RPC 函数使用 `SECURITY DEFINER` 权限，绕过 RLS 检查
     - 使用 `get_tenant_schema()` 函数获取当前用户的租户 Schema
     - 只查询当前用户所属租户的数据
  3. **更新前端查询函数**：
     - 修改 `getDriverProfiles()` 函数，使用 `supabase.rpc('get_tenant_drivers')`
     - 修改 `getAllDrivers()` 函数，使用 `supabase.rpc('get_tenant_drivers')`
  ...
  4. **处理数据结构差异**：
     ...
     - 创建转换函数，将租户 Profile 转换为 Profile 类型
- ✅ **修改内容**：
  1. 创建并应用新迁移 `create_tenant_query_functions.sql`：
     - 创建 4 个 RPC 函数用于查询租户数据
     - 使用 `get_tenant_schema()` 动态获取租户 Schema
     - 使用 `format()` 函数动态构造 SQL 查询
  2. 更新 `src/db/types.ts`：
     - 添加 `TenantRole` 类型定义
     - 添加 `TenantProfile` 接口定义
  3. 更新 `src/db/api.ts`：
     - 创建 `convertTenantProfileToProfile()` 转换函数
     - 修改 `getDriverProfiles()` 函数，使用 RPC 调用
     - 修改 `getAllDrivers()` 函数，使用 RPC 调用
     - 添加详细的日志记录
- ✅ **预期效果**：
  - 每个租户只能看到自己的司机数据
  - 租户 A 无法看到租户 B 的数据
