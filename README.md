...
**重大更新**：完成 profiles 视图到 users + user_roles 表的迁移！🎉

### 迁移背景
- profiles 视图是 users 和 user_roles 表的联合视图，用于向后兼容旧代码
- 直接查询视图会影响性能，且增加了代码复杂度
- 为了提升代码质量和查询性能，决定将所有代码迁移到直接使用 users 和 user_roles 表
...
### 迁移详情
1. **第一批到第七批**：迁移 src/db/api.ts 中的 45 个函数 ✅
2. **第八批**：迁移页面组件和服务中的 13 处使用 ✅
   - 高优先级文件：5 处（用户管理、角色更新、账号状态检查）
   - 中优先级文件：7 处（司机统计、通知服务、申请详情）
   - 低优先级文件：1 处（测试登录页面）

### 迁移效果
- ✅ 代码更清晰：直接查询 users 和 user_roles 表，逻辑更明确
- ✅ 性能提升：减少视图查询开销，提升查询效率
- ✅ 易于维护：减少了数据库对象依赖，降低维护成本
- ✅ 类型安全：使用 TypeScript 类型定义，提升代码质量

### 相关文档
- 详细迁移计划：[MIGRATION_TO_USERS_TABLE.md](./MIGRATION_TO_USERS_TABLE.md)
- 迁移进度跟踪：[PROFILES_MIGRATION_PROGRESS.md](./PROFILES_MIGRATION_PROGRESS.md)
