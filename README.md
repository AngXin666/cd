...
### 修复1：司机提交请假申请时的通知发送问题
- ✅ **修复了角色枚举值错误**：使用正确的 `'super_admin'` 替代错误的 `'boss'` 和 `'peer'`
- ✅ **重构了关联查询逻辑**：通过分步查询解决了 `driver_warehouses` 和 `manager_warehouses` 表关联问题
- ✅ **优化了查询性能**：使用 `.in()` 进行批量查询，提升查询效率
- ✅ **完善了日志记录**：添加详细的日志，便于调试和监控
- ✅ **明确了角色设计**：通过 `main_account_id` 字段区分主账号和平级账号

### 修复2：老板审批请假时的通知创建问题
- ✅ **修复了发送者信息缺失**：在应用层获取并传递完整的发送者信息（`sender_id`、`sender_name`、`sender_role`）
- ✅ **提升了代码可靠性**：添加了默认值处理，确保在任何情况下都不会出现 NULL 值
- ✅ **统一了实现逻辑**：修复了三个通知创建函数，确保一致性
- ✅ **保持了向后兼容性**：RPC 函数仍然支持旧代码的调用方式

### 修复效果
- ✅ 司机提交请假申请成功，老板和车队长正确收到通知
- ✅ 老板审批请假成功，司机正确收到审批结果通知
- ✅ 所有通知包含完整的发送者信息
- ✅ 通知接收者去重正确
- ✅ 角色查询使用正确的数据库枚举值

详细信息请查看：
- [通知服务修复报告](NOTIFICATION_FIX_REPORT.md) - 司机提交请假的修复
- [通知发送者信息修复报告](NOTIFICATION_SENDER_FIX_REPORT.md) - 老板审批请假的修复
