...
### 测试账号
| 角色 | 手机号 | 密码 | 登录账号 | Web 后台权限 |
|------|--------|------|----------|-------------|
| 超级管理员（老板） | 13800000001 | 123456 | admin | ✅ 有权限 |
| 管理员（车队长） | 13800000002 | 123456 | manager01 | ✅ 有权限 |
| 司机（纯司机） | 13800000003 | 123456 | driver01 | ❌ 无权限 |
| 司机（带车司机） | 13800000004 | 123456 | driver02 | ❌ 无权限 |

---

## 🖥️ Web 管理后台

为方便电脑端查看和管理数据，系统提供了专门的 Web 管理后台界面。

### 访问方式
- **直接访问**：https://app.appmiaoda.com/app-7cdqf07mbu9t/#/pages/web-admin/index
- **权限验证**：系统会自动验证用户权限，仅允许管理员和超级管理员访问
- **自动跳转**：如果用户无权限，系统会提示并自动跳转到工作台页面

### 权限说明
- ✅ **超级管理员（老板）**：拥有所有功能的完整访问权限
- ✅ **管理员（车队长）**：拥有所有功能的完整访问权限
- ❌ **司机**：无法访问 Web 管理后台，会被自动拦截
...
### 设计特点
- **响应式布局**：使用 Tailwind CSS 的 `md:`、`lg:`、`xl:` 断点，完美适配各种屏幕尺寸
- **卡片式设计**：统计数据使用卡片展示，美观直观
- **表格与卡片切换**：电脑端显示表格，移动端自动切换为卡片布局
- **实时数据更新**：使用 `useDidShow` 钩子确保数据实时刷新
- **友好的交互反馈**：操作成功/失败均有 Toast 提示
