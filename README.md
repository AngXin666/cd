...
### 多角色权限管理
- **司机端**：个人工作台、**综合数据统计**（当日件数、当日收入、本月件数、本月收入、出勤天数、请假天数）、仓库打卡、查看所属仓库、仓库统计详情（考勤+计件）、计件录入（录入、编辑、删除个人计件记录）、我的计件（查看个人计件记录和收入）、**快捷请假**（滚轮选择天数）、**补请假**（补录历史记录）、**离职申请**（提前天数控制）、**草稿管理**（保存、编辑、提交、删除草稿）、**智能天数计算**（自动计算请假天数）
