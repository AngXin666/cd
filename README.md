...
- ✅ **2025-12-01**：修复司机类型切换后标签不更新的问题
  - **问题描述**：
    ...
    - 标签仍然显示旧的司机类型（如"新纯司机"）
  - **根本原因**：
    - 司机类型标签显示逻辑使用的是 `getDriverDetailInfo` 返回的计算值
    - 该计算值基于实际车辆记录，而不是 `driver_type` 字段
    - 切换 `driver_type` 字段后，标签没有相应更新
  - **修复措施**：
    - 修改老板端 `getDriverType` 函数，优先使用 `driver_type` 字段
    - 修改车队长端司机类型标签显示逻辑，使用 `driver_type` 字段
    - 保留"新司机"判断逻辑（入职7天内）
  - **司机类型显示规则**：
    - 基于 `driver_type` 字段（`pure` 或 `with_vehicle`）
    - 结合入职天数判断是否为"新"司机
    - 四种类型：新纯司机、纯司机、新带车司机、带车司机
  - **相关文件**：
    - `src/pages/super-admin/user-management/index.tsx` - 修改 `getDriverType` 函数
    - `src/pages/manager/driver-management/index.tsx` - 修改司机类型标签显示逻辑

- ✅ **2025-12-01**：统一老板端和车队长端的司机类型切换按钮文本
  - **修改内容**：
    - 统一按钮文本为"切换为纯司机"和"切换为带车司机"
    - 老板端：修改 `src/pages/super-admin/user-management/index.tsx`
    - 车队长端：修改 `src/pages/manager/driver-management/index.tsx`
  - **功能说明**：
    - 老板端和车队长端都可以切换司机类型
    - 切换时会发送通知给相关人员
    - 切换后会自动刷新数据和清除缓存
...
- ✅ **2025-12-01**：添加通知系统详细调试日志和诊断指南
  ...
  - **改进措施**：
    - 在老板端审批代码中添加详细的调试日志
    - 记录查询参数、查询结果、更新过程的每一步
    - 创建完整的调试指南文档（`NOTIFICATION_DEBUG_GUIDE.md`）
  - **调试日志包含**：
    - 🔍 查询原始通知的参数（`related_id`、`type`、`current_user`）
    - 📋 查询结果的详细信息（ID、接收者、类型、关联ID、审批状态、标题、是否已读）
    - 📝 更新每条通知的详细过程
    - ✅ 更新成功/失败的统计
    - ⚠️ 未查询到通知时的可能原因分析
  - **使用方法**：
  ...
  - **诊断指南**：
    ...
    - 完整的检查清单
  - **相关文档**：
    - `NOTIFICATION_DEBUG_GUIDE.md`：完整的调试指南
