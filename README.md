...
### 多角色权限管理
- **司机端**：个人工作台、基础信息展示、今日统计、仓库打卡、当月考勤统计、查看所属仓库、仓库统计详情（考勤+计件）、我的计件（查看个人计件记录和收入）
- **普通管理端**：管理员工作台、司机管理、基础管理功能、数据汇总（查看管辖仓库的计件数据报表，支持按司机和仓库筛选，仅可删除记录）
- **超级管理端**：超级管理员控制台、系统管理、用户权限管理、仓库管理、考勤规则配置、司机仓库分配、管理员仓库分配、计件品类管理、数据汇总（查看所有仓库的计件数据报表）
...
### 权限控制
- 超级管理员：拥有所有权限，可管理所有用户和修改任何角色，可管理仓库和考勤规则，可为司机分配仓库，可为管理员分配仓库，可管理计件品类
- 普通管理员：可查看所有用户，可修改司机角色，可查看考勤记录，可管理管辖仓库的计件数据
- 司机：只能查看和修改自己的信息，只能在被分配的仓库打卡，可查看所属仓库的考勤和计件统计
...
- **数据汇总**（管理员/超级管理员）：
  ...
  - 超级管理员可查看所有仓库的数据
- **权限控制**：
  - 普通管理员：仅可查看和删除其管辖仓库的计件记录，不能修改
  - 超级管理员：可查看和删除所有仓库的计件记录
- **司机端查看**：司机可查看个人的计件记录和收入统计，支持按仓库和日期筛选
