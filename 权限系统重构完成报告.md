...
- [认证角色统计功能优化总结](./认证角色统计功能优化总结.md) - 认证、角色管理和统计功能优化总结
...
### 已创建的数据库函数
**迁移文件**: `00543_optimize_auth_roles_and_stats.sql`

#### 统计数据查询函数

| 函数名 | 权限 | 说明 |
|--------|------|------|
| get_system_stats | 仅管理员 | 获取系统总体统计数据 |
| get_user_personal_stats | 所有用户 | 获取用户个人统计数据 |
| get_warehouse_stats | 管理员和车队长 | 获取仓库统计数据 |
| get_all_warehouses_stats | 仅管理员 | 获取所有仓库统计数据 |

#### 角色管理函数

| 函数名 | 权限 | 说明 |
|--------|------|------|
| get_user_all_roles | 所有用户 | 获取用户的所有角色 |
| user_has_role | 所有用户 | 检查用户是否有指定角色 |
| add_role_to_user | 仅管理员 | 添加角色给用户 |
| remove_role_from_user | 仅管理员 | 移除用户的角色 |
| get_users_by_role | 仅管理员 | 获取指定角色的用户列表 |

#### 用户信息查询函数

| 函数名 | 权限 | 说明 |
|--------|------|------|
| get_current_user_info | 所有登录用户 | 获取当前登录用户的完整信息 |

### 系统统计数据

**函数**: `get_system_stats(p_user_id uuid)`
...
**函数**: `get_user_personal_stats(p_user_id uuid)`
...
**函数**: `get_warehouse_stats(p_warehouse_id uuid, p_user_id uuid)`
...
**函数**: `get_current_user_info()`

**返回数据**：
- 用户基本信息（ID、姓名、手机、邮箱、头像）
- 角色数组
- 权限标识（is_admin、is_manager、is_driver）
- 关联的仓库列表
- 权限详情
  - 可以管理所有数据
  - 可以管理仓库
...
```typescript
import { getCurrentUserInfo, getSystemStats, getUserPersonalStats } from '@/db/api/stats'
...
if (userInfo?.is_admin) {
  const systemStats = await getSystemStats(userInfo.user_id)
  console.log('系统统计:', systemStats)
}

// 获取个人统计
const personalStats = await getUserPersonalStats(userInfo.user_id)
...
**最后更新**: 2025-12-01  
**适用范围**: 车队管家小程序权限系统重构  
**状态**: 已完成（第一阶段 + 仓库管理 + 考勤管理 + 请假和离职管理 + 车辆管理 + 通知系统 + 性能优化 + 认证角色统计优化）
