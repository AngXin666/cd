...
实时通知功能依赖 Supabase Realtime，已为以下表启用：
- `leave_applications` - 请假申请表
- `resignation_applications` - 离职申请表
- `attendance` - 打卡记录表

如果通知不工作，请检查控制台日志中的订阅状态：
- `✅ 实时通知订阅成功！` - 表示订阅正常
- `❌ 实时通知订阅失败！` - 表示订阅失败，需要检查 Supabase 配置
...
- `🔔 尝试显示通知` - 准备显示通知
- `✅ 通过防抖检查，显示通知` - 通知将被显示
- `📢 调用 onNewNotification 回调` - 添加到通知栏
