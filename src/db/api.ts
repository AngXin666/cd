...
export async function syncAuthUsersToProfiles(): Promise<{
  ...
  try {
    console.log('🔄 开始同步 auth.users 到 profiles')
    ...
    if (tenantId) {
      // 租户用户：同步到租户 Schema
      console.log('📋 调用 sync_auth_users_to_tenant_profiles')
      const {data, error} = await supabase.rpc('sync_auth_users_to_tenant_profiles', {
        p_tenant_id: tenantId
      })
      ...
      // 中央管理员：同步到 public Schema
      console.log('📋 调用 sync_auth_users_to_profiles')
    ...
    console.log('✅ 同步成功')
    console.log('  - 同步用户数:', result.synced_count)
