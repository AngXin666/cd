...
export async function getTodayAttendanceStats() {
  try {
    ...
    const {data: records, error} = await supabase
      ...
      .select('driver_id, clock_in_time, clock_out_time')
      .gte('clock_in_time', todayStr)
      .order('id', {ascending: true})

    if (error) {
      console.error('获取今日考勤记录失败:', error)
      return {attendedCount: 0, totalCount: 0, attendanceRate: 0}
    }
    ...
    if (driversError) {
      console.error('获取司机总数失败:', driversError)
      return {attendedCount: 0, totalCount: 0, attendanceRate: 0}
    }

    const attendedCount = Array.isArray(records) ? records.length : 0
    const totalCount = Array.isArray(drivers) ? drivers.length : 0
    ...
    console.error('获取今日考勤统计异常:', error)
    return {attendedCount: 0, totalCount: 0, attendanceRate: 0}
  }
}
 ...
 * 获取今日计件统计
 */
export async function getTodayPieceWorkStats() {
  try {
    const today = new Date()
    today.setHours(0, 0, 0, 0)
    const todayStr = today.toISOString()
    ...
    if (error) {
      console.error('获取今日计件记录失败:', error)
      return {totalQuantity: 0, completedQuantity: 0, completionRate: 0}
    }

    const records = Array.isArray(data) ? data : []
    const totalQuantity = records.reduce((sum, r) => sum + (r.quantity || 0), 0)
    const completedQuantity = records.reduce((sum, r) => sum + (r.completed_quantity || 0), 0)
    ...
    console.error('获取今日计件统计异常:', error)
    return {totalQuantity: 0, completedQuantity: 0, completionRate: 0}
  }
}
...
export async function getMonthlyStats() {
  try {
    ...
    const {data: attendanceData, error: attendanceError} = await supabase
      .from('attendance_records')
      .select('driver_id, clock_in_time')
      .gte('clock_in_time', firstDayStr)
      .order('id', {ascending: true})

    if (attendanceError) {
      console.error('获取本月考勤记录失败:', attendanceError)
    }

    // 本月计件统计
    const {data: pieceWorkData, error: pieceWorkError} = await supabase
      .from('piece_work_records')
      .select('quantity, completed_quantity')
    ...
    if (pieceWorkError) {
      console.error('获取本月计件记录失败:', pieceWorkError)
    }

    const attendanceRecords = Array.isArray(attendanceData) ? attendanceData : []
    const pieceWorkRecords = Array.isArray(pieceWorkData) ? pieceWorkData : []

    const totalAttendanceDays = attendanceRecords.length
    const totalPieceWorkQuantity = pieceWorkRecords.reduce((sum, r) => sum + (r.quantity || 0), 0)
    const totalCompletedQuantity = pieceWorkRecords.reduce((sum, r) => sum + (r.completed_quantity || 0), 0)

    return {
      totalAttendanceDays,
      totalPieceWorkQuantity,
      totalCompletedQuantity
    }
  } catch (error) {
    console.error('获取本月统计数据异常:', error)
    return {
      totalAttendanceDays: 0,
      totalPieceWorkQuantity: 0,
...
export async function getVehicleStats() {
  try {
    const {data, error} = await supabase
      .from('vehicles')
      .select('is_active, current_driver_id')
      .order('id', {ascending: true})

    if (error) {
      console.error('获取车辆数据失败:', error)
      return {activeCount: 0, inactiveCount: 0, inUseCount: 0}
    }

    const vehicles = Array.isArray(data) ? data : []
    const activeCount = vehicles.filter((v) => v.is_active).length
    const inactiveCount = vehicles.filter((v) => !v.is_active).length
    const inUseCount = vehicles.filter((v) => v.is_active && v.current_driver_id).length
    ...
    console.error('获取车辆状态统计异常:', error)
    return {activeCount: 0, inactiveCount: 0, inUseCount: 0}
  }
}
...
export async function getDriverStats() {
  try {
    ...
    const {data, error} = await supabase
      .from('profiles')
      .select('id, role, created_at, resignation_date')
      .eq('role', 'driver')
      .order('id', {ascending: true})

    if (error) {
      console.error('获取司机数据失败:', error)
      return {activeCount: 0, resignedCount: 0, newThisMonth: 0}
    }

    const drivers = Array.isArray(data) ? data : []
    const activeCount = drivers.filter((d) => !d.resignation_date).length
    const resignedCount = drivers.filter((d) => d.resignation_date).length
    ...
    console.error('获取司机状态统计异常:', error)
    return {activeCount: 0, resignedCount: 0, newThisMonth: 0}
  }
}
...
export async function getPendingTasks() {
  try {
    ...
    if (leaveError) {
      console.error('获取待审批请假申请失败:', leaveError)
    }
    ...
    if (driversError) {
      console.error('获取司机数据失败:', driversError)
    }
    ...
    if (drivers.length > 0) {
      ...
      if (assignmentsError) {
        console.error('获取仓库分配数据失败:', assignmentsError)
      }

      const assignedDriverIds = Array.isArray(assignmentsData) ? assignmentsData.map((a) => a.driver_id) : []
      unassignedDrivers = drivers.filter((d) => !assignedDriverIds.includes(d.id)).length
    }
    ...
    console.error('获取待处理事项统计异常:', error)
    return {
      pendingLeaveCount: 0,
      unassignedDrivers: 0
    }
...
export async function getDashboardStats() {
  try {
    const [todayAttendance, todayPieceWork, monthlyStats, vehicleStats, driverStats, pendingTasks] = await Promise.all([
    ...
    console.error('获取仪表盘统计数据异常:', error)
    return {
      todayAttendance: {attendedCount: 0, totalCount: 0, attendanceRate: 0},
      todayPieceWork: {totalQuantity: 0, completedQuantity: 0, completionRate: 0},
      monthlyStats: {totalAttendanceDays: 0, totalPieceWorkQuantity: 0, totalCompletedQuantity: 0},
      vehicleStats: {activeCount: 0, inactiveCount: 0, inUseCount: 0},
      driverStats: {activeCount: 0, resignedCount: 0, newThisMonth: 0},
      pendingTasks: {pendingLeaveCount: 0, unassignedDrivers: 0}
    }
  }
}
