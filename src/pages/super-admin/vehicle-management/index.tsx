/**
 * 老板端 - 车辆管理页面
 * 功能：显示所有已录入的车辆信息，包括车牌号和使用人
 *
 * 使用全局缓存系统优化性能：
 * - 使用 useVehiclesCache Hook 实现缓存优先加载
 * - 实时监听数据库变更自动更新
 * - 支持离线模式显示缓存数据
 * - 所有数据变更操作后自动刷新缓存
 */
...
const VehicleManagement: React.FC = () => {
  ...
  }, [cachedVehicles, loadVehicleDetails])
  ...
  const handleSearch = useCallback(
    (value: string) => {
      setSearchText(value)
      if (!value.trim()) {
        setFilteredVehicles(vehiclesWithDrivers)
        return
      }

      const filtered = vehiclesWithDrivers.filter(
        (vehicle) =>
          ...
          <View className="p-4">
            ...
            <View className="bg-white rounded-xl p-4 mb-4 shadow-md">
              <View className="flex items-center bg-gray-50 rounded-lg px-3 py-2">
                ...
                {searchText && (
                  <View
                    className="i-mdi-close-circle text-lg text-gray-400 ml-2"
                    onClick={() => handleSearch('')}></View>
                )}
              </View>
            </View>
            ...
            <View className="grid grid-cols-3 gap-3 mb-4">
              <View className="bg-white rounded-xl p-3 shadow-md">
                ...
                <Text className="text-center text-xl font-bold text-gray-800">{vehiclesWithDrivers.length}</Text>
              </View>
              <View className="bg-white rounded-xl p-3 shadow-md">
                <View className="flex items-center justify-center mb-2">
                ...
                <Text className="text-center text-xl font-bold text-gray-800">
                  {vehiclesWithDrivers.filter((v) => v.status === 'active').length}
                </Text>
              </View>
              <View className="bg-white rounded-xl p-3 shadow-md">
                <View className="flex items-center justify-center mb-2">
                ...
                <Text className="text-center text-xl font-bold text-gray-800">
                  {vehiclesWithDrivers.filter((v) => v.driver_id).length}
                </Text>
              </View>
            </View>

            {/* 车辆列表 */}
            {isLoading && vehiclesWithDrivers.length === 0 ? (
                      ...
                      {vehicle.left_front_photo && (
                        <View className="relative w-full h-48 bg-gradient-to-br from-gray-100 to-gray-200">
                          ...
                          <View className="absolute top-3 right-3">
                            <View
                              className={`backdrop-blur rounded-full px-3 py-1 flex items-center ${statusBadge.bg}`}>
                            ...
                            <View className="flex items-center justify-between">
                              <View className="flex items-center flex-1">
                                ...
                                <View className="flex-1">
                                  ...
                                  <Text className="text-sm text-purple-900 font-medium block">
                                    {vehicle.driver_name}
                                  </Text>
