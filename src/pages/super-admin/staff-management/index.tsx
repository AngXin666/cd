...
const StaffManagement: React.FC = () => {
  ...
  // 注意：权限设置已改为跳转到 permission-config 页面，不再使用内置对话框
  ...
  const handleSetPermissions = useCallback(
    (managerId: string, managerName: string) => {
      // 跳转到权限配置页面
      navigateTo({
        url: `/pages/super-admin/permission-config/index?userId=${managerId}&userName=${encodeURIComponent(managerName)}&userRole=MANAGER`
      })
    },
    []
  )
  ...
  return (
    <View style={{background: 'linear-gradient(to bottom, #F8FAFC, #E2E8F0)', minHeight: '100vh'}}>
      ...
      {/* 权限设置已改为跳转到 permission-config 页面，不再使用内置对话框 */}
    </View>
  )
}
