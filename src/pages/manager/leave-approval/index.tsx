...
const ManagerLeaveApproval: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    // 获取所有请假申请
    const allLeaveApps: LeaveApplication[] = []
    for (const warehouseId of warehouseIds) {
      const apps = await getLeaveApplicationsByWarehouse(warehouseId)
      allLeaveApps.push(...apps)
    }
    setLeaveApplications(allLeaveApps)

    // 获取所有离职申请
    const allResignationApps: ResignationApplication[] = []
    for (const warehouseId of warehouseIds) {
      const apps = await getResignationApplicationsByWarehouse(warehouseId)
      allResignationApps.push(...apps)
    }
      ...
      <ScrollView scrollY className="box-border" style={{height: '100vh', background: 'transparent'}}>
        <View className="p-4">
          ...
          <View className="bg-gradient-to-r from-blue-900 to-blue-700 rounded-lg p-6 mb-4 shadow-lg">
            ...
            <Text className="text-blue-100 text-sm block">管理员审批工作台</Text>
          </View>
