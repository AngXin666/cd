 ...
 *
 * 使用全局缓存系统优化性能：
 * - 使用 useDriverListCache Hook 实现缓存优先加载
 * - 实时监听数据库变更自动更新
 * - 支持离线模式显示缓存数据
 */
...
const DriverManagement: React.FC = () => {
  ...
  // 司机详细信息 - 使用缓存的详情，但允许本地扩展
  const [localDriverDetails, setLocalDriverDetails] = useState<Map<string, DriverDetailInfo>>(new Map())
  // 合并缓存和本地的司机详情
  const driverDetails = useMemo(() => {
    const merged = new Map(cachedUserDetails)
    localDriverDetails.forEach((value, key) => {
      merged.set(key, value)
    })
    return merged
  }, [cachedUserDetails, localDriverDetails])

  // 司机的仓库分配信息（用于过滤） - 使用缓存的仓库映射
  const driverWarehouseMap = useMemo(() => {
    const map = new Map<string, string[]>()
    cachedUserWarehouseIdsMap.forEach((warehouseIds, driverId) => {
      map.set(driverId, warehouseIds)
    })
    return map
  }, [cachedUserWarehouseIdsMap])
  ...
  }, [loadWarehouses, loadManagerPermissions])

  // 页面显示时加载数据（批量并行查询优化）
  useDidShow(() => {
    Promise.all([loadWarehouses(), loadManagerPermissions()]).catch((error) => {
  ...
  usePullDownRefresh(async () => {
    // 清除缓存并刷新
    clearCache()
    await Promise.all([refreshCache(), loadWarehouses(), loadManagerPermissions()])
  ...
  const handleToggleDriverType = useCallback(
    async (driver: DriverWithRealName) => {
      ...
      if (success) {
        ...
        if (detail) {
          setLocalDriverDetails((prev) => new Map(prev).set(driver.id, detail))
        }
      } else {
