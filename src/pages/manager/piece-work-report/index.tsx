...
const ManagerPieceWorkReport: React.FC = () => {
      ...
      try {
        ...
        const preloadPromises = warehouses
          ...
          .map(async (warehouse) => {
            ...
            // 如果缓存中没有数据，则预加载
            if (!cached) {
              console.log(`📥 预加载仓库 ${warehouse.name} 的数据`)
              const data = await getPieceWorkRecordsByWarehouse(warehouse.id, actualStartDate, actualEndDate)
              setVersionedCache(cacheKey, data, 3 * 60 * 1000)
              console.log(`✅ 仓库 ${warehouse.name} 数据预加载完成`)
            } else {
              console.log(`⏭️ 仓库 ${warehouse.name} 数据已在缓存中`)
            }
          })

        await Promise.all(preloadPromises)
        console.log('✅ 所有仓库数据预加载完成')
      } catch (error) {
  ...
  const handleViewDriverDetail = (driverId: string) => {
    navigateTo({
      url: `/pages/manager/piece-work-report-detail/index?driverId=${driverId}&startDate=${startDate}&endDate=${endDate}&warehouseIndex=${currentWarehouseIndex}`
    })
  }
