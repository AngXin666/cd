...
import { behaviorTracker, FeatureModule, FeatureWeight } from '@/utils/behaviorTracker';
import { performanceMonitor, MetricType, PerformanceStats } from '@/utils/performanceMonitor';
                  ...
                  <View className="flex justify-between items-center mb-2">
                    <View className="flex items-center gap-2">
                      ...
                      <View className={`px-2 py-0.5 rounded ${
                        weight.weight_score >= 80 ? 'bg-red-100' :
                        weight.weight_score >= 50 ? 'bg-orange-100' : 'bg-green-100'
                      }`}>
                    ...
                    <Text className="text-lg font-bold text-primary">
                      {weight.weight_score.toFixed(1)}
                    </Text>
                  </View>
                  <View className="flex justify-between items-center text-sm">
                    <Text className="text-muted-foreground">
                      访问次数: {weight.access_count || 0}
                    </Text>
                    <Text className="text-muted-foreground">
                      缓存时间: {formatCacheTTL(weight.cache_ttl)}
                    </Text>
                  </View>
                </View>
              ))}
          ...
          {performanceStats.length === 0 ? (
            ...
            <View className="space-y-3">
              {performanceStats.slice(0, 10).map((stat, index) => (
                <View key={`${stat.metricType}-${stat.metricName}-${index}`} className="border-b border-border pb-3 last:border-0">
                  <View className="flex justify-between items-center mb-2">
                    <Text className="text-foreground font-medium">
                      {stat.metricName}
                    </Text>
