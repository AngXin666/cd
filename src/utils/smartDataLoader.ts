...
import { behaviorTracker, FeatureModule, FeatureWeight } from './behaviorTracker';
...
class SmartDataLoader {
  ...
  async preloadHighPriorityData(loaders: LoaderConfig[]) {
    ...
    const priorityModules = new Set(
      highPriorityFeatures.map(f => f.feature_module)
    );

    // 筛选出高优先级的加载器
    const priorityLoaders = loaders.filter(loader =>
      priorityModules.has(loader.featureModule)
    );
...
export function useSmartDataLoader() {
  return {
    ...
    preloadHighPriorityData: (loaders: LoaderConfig[]) =>
      smartDataLoader.preloadHighPriorityData(loaders),
    getFeatureCacheTTL: (featureModule: string) =>
      smartDataLoader.getFeatureCacheTTL(featureModule),
    getFeatureWeight: (featureModule: string) =>
      smartDataLoader.getFeatureWeight(featureModule),
