...
- `leave_approved` - 请假申请已通过（司机收到）
- `leave_rejected` - 请假申请已驳回（司机收到）
- `resignation_application_submitted` - 离职申请已提交（管理员收到）
- `resignation_approved` - 离职申请已通过（司机收到）
- `resignation_rejected` - 离职申请已驳回（司机收到）
...
**函数**：`reviewResignationApplication`
...
4. **司机端验证**：
   ...
   - **预期（通过）**：显示通知内容：
     ```
     标题：请假申请已通过
     内容：您的事假申请（2025-11-10 至 2025-11-12）已通过
     ```
   - **预期（驳回）**：显示通知内容：
     ```
     标题：请假申请已驳回
     内容：您的事假申请（2025-11-10 至 2025-11-12）已驳回，备注：请假时间与工作安排冲突
     ```
...
4. **司机端验证**：
   ...
   - **预期（通过）**：显示通知内容：
     ```
     标题：离职申请已通过
     内容：您的离职申请（期望离职日期：2025-12-31）已通过
     ```
   - **预期（驳回）**：显示通知内容：
     ```
     标题：离职申请已驳回
     内容：您的离职申请（期望离职日期：2025-12-31）已驳回，备注：请先完成交接工作
     ```
...
- `src/db/api.ts`
  ...
  - `createNotificationForAllManagers()` - 为所有管理员批量创建通知
  - `getDriverName()` - 获取司机姓名
  - `reviewLeaveApplication()` - 审批请假申请（已修改，添加通知功能）
  - `reviewResignationApplication()` - 审批离职申请（已修改，添加通知功能）

### 前端页面
- `src/pages/driver/leave/apply/index.tsx` - 司机请假申请（已修改）
- `src/pages/driver/leave/resign/index.tsx` - 司机离职申请（已修改）
- `src/pages/manager/leave-approval/index.tsx` - 管理员审批页面（使用修改后的审批函数）
- `src/pages/super-admin/leave-approval/index.tsx` - 超级管理员审批页面（使用修改后的审批函数）
...
### 1. 数据库层面
- ✅ 添加了请假和离职申请相关的通知类型（6种）
- ✅ 支持双向通知：司机→管理员、管理员→司机
...
### 3. 前端页面层面
- ✅ 修改了司机请假申请页面，提交成功后自动通知所有管理员
- ✅ 修改了司机离职申请页面，提交成功后自动通知所有管理员
- ✅ 管理员审批页面自动使用新的审批函数，审批后自动通知司机

### 4. 功能特性
- ✅ 通知内容包含完整信息（司机姓名、时间、天数、事由、审批结果等）
- ✅ 支持多个管理员同时接收通知
- ✅ 普通管理员和超级管理员都能收到通知
- ✅ 完整的双向通知流程：
  - 司机提交申请 → 通知所有管理员
...
**修复完成时间**：2025-11-05  
**相关提交**：
- `f2d5977` - 修复司机请假后双管理员端通知栏显示问题
- `1d27d52` - 完善通知系统：管理员审批后自动通知司机
