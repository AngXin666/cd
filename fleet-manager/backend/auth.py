...
# 从 common.py 导入密码函数，解决循环导入问题
# 同时重新导出以保持向后兼容性
from common import hash_password, verify_password
# 导入 crud 模块（循环依赖已通过 common.py 解决）
import crud
...
# hash_password 和 verify_password 函数已从 common.py 导入
# 保持向后兼容性，其他模块可以继续从 auth 导入这些函数
...
# 预定义的角色检查依赖
# 只有老板可以访问（老板是系统最高权限角色）
require_boss = require_roles([UserRole.BOSS])

# 老板和调度可以访问（管理员级别）
require_admin = require_roles([UserRole.PEER_ADMIN, UserRole.BOSS])

# 老板和车队长可以访问
require_manager_or_boss = require_roles([UserRole.MANAGER, UserRole.BOSS])

# 车队长、调度、老板可以访问（管理权限）
require_management = require_roles([UserRole.MANAGER, UserRole.PEER_ADMIN, UserRole.BOSS])
...
def get_creatable_roles(current_role: UserRole) -> List[UserRole]:
    ...
    权限规则：
    - BOSS: 可以创建所有角色（包括其他 BOSS）
    - PEER_ADMIN: 可以创建 MANAGER, DRIVER
    - MANAGER: 可以创建 DRIVER
    - DRIVER: 无法创建任何角色

    注意：SUPER_ADMIN 角色已被移除，BOSS 现在是最高权限角色
...
def can_manage_user(manager_role: UserRole, target_role: UserRole) -> bool:
    ...
    # BOSS 可以管理所有角色（包括其他 BOSS）
    if manager_role == UserRole.BOSS:
        return True

    # 其他角色：管理者权限等级必须高于目标用户
...
def require_boss_for_high_roles(
    ...
    当目标角色是老板时，只有老板才能执行操作。
    删除超级管理员后，老板成为系统最高权限角色。
    ...
    Raises:
        PermissionError: 当非老板尝试操作老板角色时

    Example:
        # 创建用户时检查
        require_boss_for_high_roles(user_data.role, current_user, "创建")

        # 修改用户时检查
        require_boss_for_high_roles(target_user.role, current_user, "修改")

    Requirements: 3.1 - 删除超级管理员后，老板成为最高权限角色
    """
    # 高权限角色：老板（系统最高权限）
    if target_role == UserRole.BOSS:
        if current_user.role != UserRole.BOSS:
            ...
            raise PermissionError(
                ...
                message=f"只有老板可以{operation}老板角色"
            )
