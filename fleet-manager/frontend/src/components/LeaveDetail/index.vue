<template>
  ...
  <view class="leave-detail-page">
    ...
    <!-- 加载状态 -->
    <view v-if="loading" class="loading-container" :class="{ 'with-nav': showNavBar }">
 ...
 * 显示申请详情，支持审批操作
 * 
 * @requirements 2.1 - 创建公共组件
 * @requirements 2.2 - 通过 props 传入请假数据和审批权限
 * @requirements 2.3 - 保持原有审批功能不变
 */
...
interface Props {
  ...
  showNavBar?: boolean
  /** 审批成功后是否返回上一页（否则刷新详情） */
  navigateBackOnApprove?: boolean
}

const props = withDefaults(defineProps<Props>(), {
  showNavBar: true,
  navigateBackOnApprove: true,
})
...
async function loadApplication(): Promise<void> {
  loading.value = true
  try {
    const data = await getLeaveApplication(props.leaveId)
...
async function doApprove(status: LeaveStatus): Promise<void> {
  try {
    ...
    await approveLeaveApplication(props.leaveId, {
      status,
    ...
    // 根据配置决定审批后行为
    if (props.navigateBackOnApprove) {
      setTimeout(() => {
        uni.navigateBack()
      }, 1500)
    } else {
