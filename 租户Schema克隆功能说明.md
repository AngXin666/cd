...
**V3 简化版**：当前版本只克隆表结构和基本约束（主键、唯一约束），不克隆外键、触发器、函数和 RLS 策略，以确保克隆的可靠性和稳定性。
